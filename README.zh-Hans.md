...
`720` 仅为示例，可将 `length` 改成所需帧数。getnative 和已选择的音轨转换仍会检查或处理完整来源，最终 MKV 中的来源音轨、软字幕和章节也不会同步截短；HDR10+ 会因完整来源时间轴与 VPy 输出不一致而被省略，这也不能作为可靠的完整 Dolby Vision 测试。若要测试全部压制流程，应使用视频、音频、字幕、章节和动态元数据已经同步截短的短 MKV。
