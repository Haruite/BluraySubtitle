...
- 主播放列表支持编辑混流命令（`remux_cmd`）。每个已选主播放列表必须且只能对应一条非空命令，并按照界面当前显示的顺序执行；同一分卷选择多个主播放列表时也一样。视频、音频和字幕选择始终以“编辑轨道”为准；命令只显示这些选择的占位符，手工填写的选轨参数不会生效。
- 写入前，程序会推导全部命令输出和最终分集文件名。输出数量必须与可见分集行一致；路径重复或文件已经存在都会报错。
- 最终分集名称严格使用界面中显示的名称，无效文件名直接报错。
- 直接混流多片段 MPLS 前，Remux 会根据命令的实际时间区间确定参与混流的 M2TS，并只在这些片段之间比较已选 PID 到本地轨道 ID 的映射；已选 PID 缺失或本地轨道 ID 不一致时直接进入轨道对齐 fallback。被 `--split parts` 排除的片段不参与检查，未选择轨道的缺失本身不会触发 fallback；如果该缺失使某个已选 PID 的本地 ID 前移，仍会因已选映射不一致而触发 fallback。主命令及 README 记录的 fallback 无法生成全部计划输出时，Remux 会报错停止，不会使用输出目录中的无关文件代替。
- Remux 保持已选择的有损音轨不变。“将无损音轨转换为 FLAC”默认启用，其启动状态可在“高级”中配置。独立 `flac` 与 FFmpeg 的压缩等级也可在此配置，两者默认都是 8。
- FLAC 不能保留 DTS:X 和 TrueHD Atmos 的对象元数据，因此“Remux 时将 DTS:X 和 TrueHD Atmos 转换为 FLAC”是单独的高级选项，并默认关闭。转换失败时保留原音轨。时长缩短超过 0.1 秒会提示；超过可配置阈值时删除 FLAC 并保留原音轨，阈值默认为 1 秒。详细原理见[媒体处理方案与工具选型](docs/development/media-pipeline-and-tool-selection.zh-Hans.md)。
- 原盘 Remux 中选择的字幕始终作为软字幕轨内封到对应的正片 MKV，不会内嵌到画面，也不会输出为外挂字幕文件。
- 启用“混流 Dolby Vision”时，Remux 会将兼容的基础层和增强层合并为 Profile 8.1；关闭时不包含增强层。
- 混流完成后，程序会把“编辑轨道”保存的语言应用到实际包含的视频、音频和字幕轨道并验证结果；映射、工具或语言验证失败时会中止当前任务并删除该任务新建的主输出。程序还会检查输出轨道数量及 MKVToolNix 数据包统计；这类检查结果不会中断后续 Remux，任务完成后会统一警告。
...
- 压制参数支持：
  ...
  - 勾选“检测花屏/坏帧”时，Encode 会在成品完成后重新运行本次压制实际使用的 VPy，并通过 PSNR 将全部参考帧与成品 MKV 逐帧比较；帧数、速度、百分比和预计剩余时间每 15 秒输出到终端。JSON 报告保存在 **`<所选输出目录>/<来源文件夹名>/FrameCheck`**。这相当于压制结束后额外执行一次完整 VPy 渲染，可能需要视频时长的几倍时间
- 无损音轨使用“编辑轨道”中逐轨显示的 FLAC/AAC/Opus 选项。
- 字幕封装：外挂 / 内挂 / 内嵌
...
- 分集关联 SP 会先使用 MPLS 的 `stream_id` PID 参考；直接映射缺失或不一致时，先生成 PID 对齐输出，再按其规范 PID 映射追加。`properties.number` 绝不会被当作 transport PID。
- 只有准确的计划输出真实存在才算成功。任何已选择行失败都会中止任务，并只清理该任务新建的不完整输出；追加 SP 的分集文件只有在新文件完成并验证后才会被替换。

#### B）轨道对齐与缺轨修复

当播放列表中的不同片段具有不同轨道布局时，直接混流可能失败。主 MPLS 与有 MPLS 的 SP 选择都保存为“轨道类型 + MPEG PID”；GUI 行顺序和首片流索引都不属于执行契约。没有 MPLS、仅由一个 M2TS 构成的 SP 继续保存源文件本地轨道 ID，并从对应 CLPI 文件读取轨道语言：

1. 主行和 SP 行的“编辑轨道”都直接读取 MPLS 流表。开始执行后，Service 才分析 MPLS，把已选 PID 映射为 mkvmerge 轨道 ID；任一已选 PID 缺失时，会在逐个检查 M2TS 之前进入回退。
2. 播放范围来自 `Chapter(mpls_path).in_out_time`；使用 `--split parts:start-end` 时只检查与时间区间重叠的 M2TS。
3. 长时间混流开始前，逐个用 mkvmerge 直接分析这些 M2TS。已选 PID 缺失，或同一 PID 的本地轨道 ID 与 MPLS 参考不一致时，立即进入 PID 对齐回退。
4. 缺少的已选轨道使用 tsMuxer 恢复，恢复后的轨道顺序采用 MPLS 要求的 PID 顺序。
5. tsMuxer 无法补齐任何缺失的已选轨道（包括音轨）时明确失败，不生成合成替代轨道。
6. 修复后的 PID 集合必须与参考布局完全一致。只有一个片段时先用 `mkvpropedit` 原地写入封面，再直接移动到计划输出，不执行第二次完整混流；多个片段使用 `--append-mode file` 顺序拼接，使所有轨道使用同一个前序文件时间戳边界。
7. 主输出、独立 SP、SP 音频中间混流及分集关联 SP 的命令共用这套预检／回退契约。图片抽取不选择或混流媒体轨道，因此排除在外。随后写入已配置的轨道语言与章节，并检查命令结果和最终元数据。
...
- 自动生成的默认 VPy 把处理后的 `res` 设为输出索引 `0`，把原始 `src8` 设为输出索引 `1`。VSEdit 的预览与截图快捷键见 [Encode／VapourSynth Wiki](docs/wiki/Video-Encoding-and-VapourSynth.zh-Hans.md)；这项实时预览不同于压制完成后由“输出对比图”生成的 PNG 文件。
- 默认脚本路径为 `vpy.vpy`。
- 自动生成的默认脚本不会自动处理隔行视频，因为真隔行、胶转磁和混合 cadence 需要不同方案；请按 [Encode／VapourSynth Wiki](docs/wiki/Video-Encoding-and-VapourSynth.zh-Hans.md) 说明使用自定义 VPy。
