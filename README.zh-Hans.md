...
- 压制参数支持：
  ...
  - 启动时的编码器、位深、预设和预设参数来自“高级”设置；启动后每个任务仍以 Encode
    页面当前可见的控件值为准
  - 启动时的无损音频目标、字幕封装方式、getnative 和输出对比图复选框同样来自“高级”设置，并可在
    启动任务前继续修改
...
压制会自动把兼容的来源色彩与 HDR 元数据写入输出；x265 10/12 位输出还会自动保留 HDR10+。选择原盘或 Remux 来源中的 Dolby Vision 时，x265 10/12 位输出会将其保留为 Profile 8.1；x264 和 x265 8 位不能保留 Dolby Vision，SVT-AV1 会明确提示并省略。自动元数据处理出现问题时会给出警告，便于用户检查输出。
