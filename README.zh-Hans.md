...
- 压制参数支持：
  ...
  - 勾选“输出对比图”时，每个压制视频都会把源和压制成品的同一帧保存为 PNG，路径为 **`<所选输出目录>/<来源文件夹名>/Compare`**。BluraySubtitle 会先扫描压制成品；帧数、解码速度、百分比和预计剩余时间每 15 秒输出到终端，不再替换 Encode 底部按钮的阶段文字。源文件达到成品帧数后便立即停止扫描
  - 勾选“检测花屏/坏帧”时，Encode 会在成品完成后重新运行本次压制实际使用的 VPy，并通过 PSNR 将全部参考帧与成品 MKV 逐帧比较；帧数、速度、百分比和预计剩余时间同样按上述间隔输出到终端。JSON 报告保存在 **`<所选输出目录>/<来源文件夹名>/FrameCheck`**。这相当于压制结束后额外执行一次完整 VPy 渲染，可能需要视频时长的几倍时间
...
压制进度提示中的正片会独立于用户可编辑的输出名，按实际处理顺序显示为 `EP01`、`EP02` 等；选中的 SP 同样按 table3 的可见勾选顺序显示为 `SP01`、`SP02` 等，不受未选中行影响。只需复制的图片目录和抽取出的 IGS 菜单资源也各自占用一个 SP 顺序号。原盘准备阶段的 SP 混流和真正执行视频压制时会显示对应的原始输出名，其余 SP 阶段使用简短的顺序编号。提示会明确区分视频／VapourSynth 元数据分析、视频压制、音轨提取、静音／重复音轨分析、逐轨音频转换、最终 Matroska 混流和最终元数据验证。
