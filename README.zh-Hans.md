...
1. `select/选择` 列控制该 SP 行是否参与混流。
2. MPLS 行统一使用 MPLS 处理；只有没有 MPLS 的行才使用 M2TS 处理。
3. `table3` 行顺序先按 BDMV 分卷排列，再按 MPLS 名排列，最后将没有 MPLS 的行按 M2TS 名排列。
...
8. 特例 2：如果 MPLS 只有一个 m2ts 且该 m2ts 只有 1 帧，默认勾选，输出为 `BD_Vol_{bdmv_vol}_SP{n}.png`。
9. 特例 3：如果 MPLS 有多个 m2ts 且每个都只有 1 帧，默认勾选，输出为文件夹 `BD_Vol_{bdmv_vol}_SP{n}`，文件名格式 `{m}-{m2ts_name}.png`（`m` 从 1 开始补 0，`m2ts_name` 不带 `.m2ts`）。
...
13. 字幕轨道同理，只选一条时输出为原始后缀，选多条时输出为 `.mks`。
...
18. 最后再扫描一遍“未被任何 MPLS 覆盖”的 m2ts 并追加到 `table3`：
    - m2ts 类型包括 video / audio_only / igs_menu / subtitle_only / audio_with_subtitle / private_or_other / mixed_non_video / unknown；
