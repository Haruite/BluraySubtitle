...
- 压制参数支持：
  ...
  - 手动脚本使用的算法会按源高度缩放搜索与曲线尾段边界：始终排除较少见的 535p 至 545p 误报带，并排除高于“源高度 × 1040 / 1080”的候选值。对 2160p 源，1080p 附近的候选仍然有效，因为由 1080p 放大到 2160p 的素材很常见，目前也没有确认 UHD 中存在对应的干扰带
  - 勾选“自动裁剪黑边”时，Encode 会分析多个时间点并应用一组保守的固定裁剪值。黑边随时间变化时会优先保留任一样本用到的像素
  - 勾选“输出对比图”时，每个压制视频都会把源和压制成品的同一帧保存为 PNG，路径为 **`<所选输出目录>/<来源文件夹名>/Compare`**。本机实测 52 秒的 1080p 素材约耗时 36 秒，34 秒的裁剪后 4K Dolby Vision 素材约耗时 80 秒，分别约为视频时长的 0.7 倍和 2.4 倍；这只是量级参考，实际时间还会受源解码、VPy 滤镜、分辨率和存储速度影响
  - 勾选“检测花屏/坏帧”时，Encode 会在成品完成后重新运行本次压制实际使用的 VPy，并通过 PSNR 将全部参考帧与成品 MKV 逐帧比较；JSON 报告保存在 **`<所选输出目录>/<来源文件夹名>/FrameCheck`**。这相当于压制结束后额外执行一次完整 VPy 渲染，重滤镜或 4K 素材可能增加数分钟乃至更久；发现疑似异常或检测工具出错时会保留成品，并以警告状态完成该行
