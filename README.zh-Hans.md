...
- 直接混流多片段 MPLS 前，Remux 会比较每个 PlayItem 中已选 PID 到本地轨道 ID 的映射；发现不一致时直接进入轨道对齐 fallback，避免 MKVToolNix 在后续分集输出中生成空轨道。主命令及 README 记录的 fallback 无法生成全部计划输出时，Remux 会报错停止，不会使用输出目录中的无关文件代替。
...
5. 修复后的 PID 集合必须与参考布局完全一致。只有一个片段时先用 `mkvpropedit` 原地写入封面，再直接移动到计划输出，不执行第二次完整混流；多个片段使用 `--append-mode file` 顺序拼接，使所有轨道使用同一个前序文件时间戳边界。
