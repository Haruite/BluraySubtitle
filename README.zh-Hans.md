...
- “编辑轨道”按逻辑轨道逐行显示整条播放列表的 PID 和状态摘要，悬停可查看各 PlayItem 的明细。轨道参数在片段之间发生不兼容变化时，该行会被禁用。逻辑轨道可以在不同片段使用不同 PID，也可以只在部分片段存在；默认语言取它第一次出现的明确语言，后续语言变化仅作提示。
- “高级”中的“允许非视频轨道部分缺失”默认不勾选。启用后，如果已选音频或字幕轨道仅在部分 M2TS 中物理缺失且 tsMuxer 也无法恢复，对应区间会作为时间线空档继续处理；视频轨道缺失或已选轨道在整个输出中都不存在时仍会报错。
- Remux 保持已选择的有损音轨不变。“将无损音轨转换为 FLAC”默认启用，其启动状态可在“高级”中配置。独立 `flac` 与 FFmpeg 的压缩等级也可在此配置，两者默认都是 8。
- FLAC 不能保留 DTS:X 和 TrueHD Atmos 的对象元数据，因此“Remux 时将 DTS:X 和 TrueHD Atmos 转换为 FLAC”是单独的高级选项，并默认关闭。音频转换会在 Matroska 中保留播放列表空档，不用静音填充；带有这种空档的独立音频输出会被拒绝。间隙检测完成后，每个 Remux 都会在 MKV 旁生成对应的 `.audio-gaps.json`，供以后从 Remux 来源压制时直接复用检测结果；存在空档时写入相应区间，音轨连续时生成有效的空记录文件。转换失败时保留原音轨。任一连续区间缩短超过 0.1 秒会提示；单个区间的最大损失超过可配置阈值时放弃转换轨，阈值默认为 1 秒。详细原理见[媒体处理方案与工具选型](docs/development/media-pipeline-and-tool-selection.zh-Hans.md)。
...
5. 剧集模式有两条相互独立的精确匹配规则。非主 MPLS 的完整 M2TS detail 与唯一一个已选主 MPLS 完全一致时，它提供的不重复音频和字幕轨道会成为该主 MPLS 的可选轨道，重复的 SP 行默认不勾选。否则，只有完整 detail 与唯一一集完全一致且提供新物理轨道的 SP 才默认勾选并只关联到该集。不同播放列表的轨道按 M2TS/PID 对应关系比较，不单独按 PID 或槽位判断。跨越多个分集输出、但不与整条主 MPLS 完全一致的 SP 保持普通 SP，不会附加到多个输出。
...
轨道对齐回退可以在 PID 变化或中间暂时缺轨时保持同一条逻辑轨道，且不会生成静音替代轨。用户选择音频转换时，仍在 Remux 成功后的独立后处理阶段执行。
