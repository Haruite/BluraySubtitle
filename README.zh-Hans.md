...
- 顶部语言栏和底部执行按钮固定，其余内容可纵向滚动；三张表按原盘、正片输出和 SP 分区，并显示操作说明；拖动表格底部的调节条可改变高度。
...
- 压制保留来源 CFR／VFR 视频时间轴和音画起始偏移，默认 VPy 的内嵌字幕使用同一时间轴。VPy 处理须保持帧对应关系，[开头截取测试](#如何快速测试压制不跑完整片)除外。
...
必须保持 `first=0`，这样“输出对比图”使用的来源与成品帧号才仍然对应。测试保留前 720 帧的来源时间戳；VFR 的实际测试时长由这些时间戳决定。该写法只会缩短处理后的视频：getnative 和已选择的音轨转换仍会检查或处理完整来源，最终 MKV 中的来源音轨、软字幕和章节也不会同步截短；HDR10+ 会因完整来源时间轴与 VPy 输出不一致而被省略，这也不能作为可靠的完整 Dolby Vision 测试。若要测试全部压制流程，应使用视频、音频、字幕、章节和动态元数据已经同步截短的短 MKV。
