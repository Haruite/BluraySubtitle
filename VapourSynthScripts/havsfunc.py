# Holy’s ported AviSynth functions for VapourSynth. Modified by MIR.
# Based on 7f0a9a7a37b60a05b9f408024d203e511e544a61: remove vsutil; use mirvsfunc and lexpr; try to ensure compatibility with older and latest versions.
# It’s not recommended to use the opencl argument of any function. Opencl version plugins of EEDI3 and NNEDI3 not have much performance improvement; NLM_CUDA is faster than KNLMeans. Suggest use CPU version if you don’t use newer NVIDIA GPU. 

__version__ = "33-7f0a9a7-AC-4"
__all__ = [ "daa", "daa3mod", "mcdaa3", "santiag", "FixChromaBleedingMod", "Deblock_QED",
            "DeHalo_alpha", "EdgeCleaner", "FineDehalo", "FineDehalo2", "YAHR", "HQDeringmod",
            "QTGMC", "smartfademod", "srestore", "dec_txt60mc", "ivtc_txt30mc", "ivtc_txt60mc",
            "logoNR", "Vinverse", "Vinverse2", "LUTDeCrawl", "LUTDeRainbow",
            "Stab", "GSMC", "GrainStabilizeMC", "MCTemporalDenoise", "SMDegrain", # GSMC = GrainStabilizeMC
            "STPresso", "bbmod", "SigmoidInverse", "SigmoidDirect",
            "GrainFactory3", "InterFrame",
            "FixColumnBrightness", "FixRowBrightness", "FixColumnBrightnessProtect", "FixRowBrightnessProtect", "FixColumnBrightnessProtect2", "FixRowBrightnessProtect2",
            "SmoothLevels", "FastLineDarkenMOD", "Toon", "LSFmod",
            "TemporalDegrain", "aaf",
            "AverageFrames", "AvsPrewitt", "Bob", "ChangeFPS", "Gauss", "Clamp", "mt_clamp", "NLMeans", "DFTTestSelect", "Overlay", # Clamp = mt_clamp, KNLMeansCL = NLMeans
            "Padding", "Resize", "SCDetect", "Weave", "ContraSharpening", "MinBlur", "sbr", "sbrV", "DitherLumaRebuild",
            "mt_expand_multi", "mt_inpand_multi", "mt_inflate_multi", "mt_deflate_multi"
        ]
...
import numbers
from mirvsfunc import RgTools, scale_value, GetFrameProp, BM3D, SplitPlanes


rg11 = [1,2,1, 2,4,2, 1,2,1]
rg19 = [1,1,1, 1,0,1, 1,1,1]
rg20 = [1,1,1, 1,1,1, 1,1,1]
PlanesType = Optional[Union[int, Sequence[int]]]


def daa(c: vs.VideoNode, nsize: Optional[int] = None, nns: Optional[int] = None, qual: Optional[int] = None, pscrn: Optional[int] = None, int16_prescreener: Optional[bool] = None, int16_predictor: Optional[bool] = None, exp: Optional[int] = None, opencl: bool = False, device: Optional[int] = None) -> vs.VideoNode:
    '''
    ...
    if opencl:
        nnedi3 = partial(core.nnedi3cl.NNEDI3CL, device=device, **nnedi3_args)
    else:
        nnedi3 = partial(core.znedi3.nnedi3, int16_prescreener=int16_prescreener, int16_predictor=int16_predictor, exp=exp, **nnedi3_args)

    nn = nnedi3(c, field=3)
    dbl = core.std.Merge(nn[::2], nn[1::2])
    dblD = core.std.MakeDiff(c, dbl)
    shrpD = core.std.MakeDiff(dbl, dbl.std.Convolution(matrix=rg20 if c.width > 1100 else rg11))
    DD = RgTools(shrpD).Repair(dblD, 13)
    return core.std.MergeDiff(dbl, DD)


def daa3mod(c1: vs.VideoNode, nsize: Optional[int] = None, nns: Optional[int] = None, qual: Optional[int] = None, pscrn: Optional[int] = None, int16_prescreener: Optional[bool] = None, int16_predictor: Optional[bool] = None, exp: Optional[int] = None, opencl: bool = False, device: Optional[int] = None) -> vs.VideoNode:
...
def mcdaa3(input: vs.VideoNode, nsize: Optional[int] = None, nns: Optional[int] = None, qual: Optional[int] = None, pscrn: Optional[int] = None, int16_prescreener: Optional[bool] = None, int16_predictor: Optional[bool] = None, exp: Optional[int] = None, opencl: bool = False, device: Optional[int] = None) -> vs.VideoNode:
...
def santiag(c: vs.VideoNode, strh: Union[float, int] = 1, strv: Union[float, int] = 1, type: str = 'nnedi3', nsize: Optional[int] = None, nns: Optional[int] = None, qual: Optional[int] = None, pscrn: Optional[int] = None, int16_prescreener: Optional[bool] = None, int16_predictor: Optional[bool] = None, exp: Optional[int] = None, aa: Optional[int] = None, alpha: Optional[float] = None, beta: Optional[float] = None, gamma: Optional[float] = None, nrad: Optional[int] = None, mdis: Optional[int] = None, vcheck: Optional[int] = None, fw: Optional[int] = None, fh: Optional[int] = None, halfres: bool = False, typeh: Optional[str] = None, typev: Optional[str] = None, opencl: bool = False, device: Optional[int] = None) -> vs.VideoNode:
    '''
    santiag v1.6
    Simple antialiasing
    ...
    def santiag_stronger(c: vs.VideoNode, strength: int, type: str) -> vs.VideoNode:
        ...
        if opencl:
            nnedi3 = partial(core.nnedi3cl.NNEDI3CL, device=device, **nnedi3_args)
            eedi3 = partial(core.eedi3m.EEDI3CL, device=device, **eedi3_args)
        else:
            nnedi3 = partial(core.znedi3.nnedi3, int16_prescreener=int16_prescreener, int16_predictor=int16_predictor, exp=exp, **nnedi3_args)
            eedi3 = partial(core.eedi3m.EEDI3, **eedi3_args)
...
def FixChromaBleedingMod(input: vs.VideoNode, cx: int = 4, cy: int = 4, thr: float = 4.0, strength: float = 0.8, blur: bool = False) -> vs.VideoNode:
    ...
    vch = Tweak(input, sat=thr).std.ShufflePlanes(2, vs.GRAY)

    if blur:
        area = vch.std.Convolution(matrix=rg11)
    else:
        area = vch
    ...
    if not blur:
        mask = mask.std.Convolution(matrix=rg11)
    ...
    input_c = Tweak(input.resize.Spline36(src_left=cx, src_top=cy), sat=strength) # v1.35 use Spline36

    # combine both images using the mask
    fu = core.std.MaskedMerge(input.std.ShufflePlanes(1, vs.GRAY), input_c.std.ShufflePlanes(1, vs.GRAY), mask)
    fv = core.std.MaskedMerge(input.std.ShufflePlanes(2, vs.GRAY), input_c.std.ShufflePlanes(2, vs.GRAY), mask)
    return core.std.ShufflePlanes([input, fu, fv], planes=[0, 0, 0], colorfamily=input.format.color_family)

# Update to AVS 2016-6-19 https://github.com/realfinder/AVS-Stuff/blob/cd4e6e41ba7c50be2e824a22d45935bb50986cb7/avs%202.5%20and%20up/Deblock_QED_MT2.avsi
# Add exdeblock1, exdeblock2, prefer_lexpr
def Deblock_QED(clp: vs.VideoNode, quant1: int = 24, quant2: int = 26, aOff1: int = 1, bOff1: int = 2, aOff2: int = 1, bOff2: int = 2, uv: int = 3, exdeblock1: Optional[vs.VideoNode] = None, exdeblock2: Optional[vs.VideoNode] = None, prefer_lexpr: bool = False) -> vs.VideoNode:
    '''
    ...
    w = clp.width
    h = clp.height
    padX = 0 if w%8==0 else 8-w%8 # padX = 8 - w % 8 if w & 7 else 0
    padY = 0 if h%8==0 else 8-h%8 # padY = 8 - h % 8 if h & 7 else 0
    
    if padX or padY:
        clp = Padding(clp, right=padX, bottom=padY)

    # block
    if prefer_lexpr:
        block = clp[0].akarin.Expr(f"X 1 + 8 % 1 < X 8 % 1 < Y 1 + 8 % 1 < Y 8 % 1 < or or or {peak} 0 ?", opt=1).std.Loop(times=clp.num_frames)
    else:
    ...
    normal = exdeblock1 if exdeblock1 else clp.deblock.Deblock(quant=quant1, aoffset=aOff1, boffset=bOff1, planes=[0, 1, 2] if uv != 2 and not isGray else [0])
    strong = exdeblock2 if exdeblock2 else clp.deblock.Deblock(quant=quant2, aoffset=aOff2, boffset=bOff2, planes=[0, 1, 2] if uv != 2 and not isGray else [0])
    ...
    normalD2 = core.akarin.Expr([normalD, block], expr=[expr] if uv >= 3 or isGray else [expr, ''])
    strongD2 = core.akarin.Expr([strongD, block], expr=[expr] if uv >= 3 or isGray else [expr, ''])
    ...
    sw = strongD2.width
    sh = strongD2.height
    remX = 0 if sw%16==0 else 16-sw%16 # 16 - sw % 16 if sw & 15 else 0
    remY = 0 if sh%16==0 else 16-sh%16 # 16 - sh % 16 if sh & 15 else 0
    if remX or remY:
        strongD2 = Padding(strongD2, right=remX, bottom=remY)
    expr = f'x {neutral} - 1.01 * {neutral} +'
    strongD3 = strongD2.akarin.Expr(expr=[expr] if uv >= 3 or isGray else [expr, '']).dctf.DCTFilter(factors=[1, 1, 0, 0, 0, 0, 0, 0], planes=planes).std.Crop(right=remX, bottom=remY)
    ...
    strongD4 = core.akarin.Expr([strongD3, normalD2], expr=[expr] if uv >= 3 or isGray else [expr, ''])
...
def DeHalo_alpha(clp: vs.VideoNode, rx: float = 2.0, ry: float = 2.0, darkstr: float = 1.0, brightstr: float = 1.0, lowsens:  Union[int, float] = 50, highsens: Union[int, float] = 50, ss: float = 1.5) -> vs.VideoNode:
    '''
    ...
    if clp.format.color_family != vs.GRAY:
        clp_orig = clp
        clp = clp.std.ShufflePlanes(0, vs.GRAY)
    else:
        clp_orig = None
    ...
    # are = core.akarin.Expr([clp.std.Maximum(), clp.std.Minimum()], expr=['x y -'])
    # ugly = core.akarin.Expr([halos.std.Maximum(), halos.std.Minimum()], expr=['x y -'])
    # so = core.akarin.Expr([ugly, are], expr=f'y x - y 0.000001 + / {scale_value(255, 8, bits)} * {scale_value(lowsens, 8, bits)} - y {scale_value(256, 8, bits)} + {scale_value(512, 8, bits)} / {highsens / 100} + *')
    
    # if clp.format.sample_type == vs.FLOAT:
    #     so = so.akarin.Expr('x 0 max 1 min')

    # lets = core.std.MaskedMerge(halos, clp, so)
    lets = core.akarin.Expr(
        [clp, halos], 
        expr=[
            'x[-1,-1] x[-1,0] max x[-1,1] max x[0,-1] max x[0,0] max x[0,1] max x[1,-1] max x[1,0] max x[1,1] max '
            'x[-1,-1] x[-1,0] min x[-1,1] min x[0,-1] min x[0,0] min x[0,1] min x[1,-1] min x[1,0] min x[1,1] min - are! '
            'y[-1,-1] y[-1,0] max y[-1,1] max y[0,-1] max y[0,0] max y[0,1] max y[1,-1] max y[1,0] max y[1,1] max '
            'y[-1,-1] y[-1,0] min y[-1,1] min y[0,-1] min y[0,0] min y[0,1] min y[1,-1] min y[1,0] min y[1,1] min - ugly! '
            f'are@ ugly@ - are@ 0.000001 + / {scale_value(255, 8, bits)} * {scale_value(lowsens, 8, bits)} - are@ {scale_value(256, 8, bits)} + {scale_value(512, 8, bits)} / {highsens / 100} + * so! '
            f'so@ {scale_value(1, 32, bits, range=1)} / 0 1 clip dup 1 swap - y * swap x * +'
        ]
    )
    if ss <= 1:
        remove = RgTools(clp).Repair(lets, 1)
    else:
        remove = core.akarin.Expr([core.akarin.Expr([clp.resize.Lanczos(m4(ox * ss), m4(oy * ss)),
    ...
    them = core.akarin.Expr([clp, remove], expr=[f'x y < x x y - {darkstr} * - x x y - {brightstr} * - ?'])
...
def EdgeCleaner(c: vs.VideoNode, strength: int = 10, rep: bool = True, rmode: int = 17, smode: int = 0, hot: bool = False) -> vs.VideoNode:
    ...
    if c.format.color_family != vs.GRAY:
        c_orig = c
        c = c.std.ShufflePlanes(0, vs.GRAY)
    else:
        c_orig = None
    ...
    if rep:
        main = RgTools(main).Repair(c, rmode)

    mask = AvsPrewitt(c).akarin.Expr(expr=[f'x {scale_value(4, 8, bits)} < 0 x {scale_value(32, 8, bits)} > {peak} x ? ?']).std.InvertMask().std.Convolution(matrix=rg20)
    
    final = core.std.MaskedMerge(c, main, mask)

    if hot:
        final = RgTools(final).Repair(c, 2)
    
    if smode > 0:
        clean = RgTools(c).RemoveGrain(17)
        diff = core.std.MakeDiff(c, clean)
        mask = RgTools(AvsPrewitt(diff.std.Levels(min_in=scale_value(40, 8, bits), max_in=scale_value(168, 8, bits), gamma=0.35))).RemoveGrain(7)

        mask = core.akarin.Expr(mask, expr=f'x {scale_value(4, 8, bits)} < 0 x {scale_value(16, 8, bits)} > {peak} x ? ?')
...
def FineDehalo(src: vs.VideoNode, rx: float = 2.0, ry: Optional[float] = None, thmi: int = 80, thma: int = 128, thlimi: int = 50, thlima: int = 100, darkstr: float = 1.0, brightstr: float = 1.0, showmask: int = 0, contra: float = 0.0, excl: bool = True, edgeproc: float = 0.0, mask: Optional[vs.VideoNode] = None) -> vs.VideoNode:
    '''
    ...
    limitExpr = '0 max 1 min' if isFloat else ''
    ...
    strong = edges.akarin.Expr(expr=f'x {scale_value(thmi, 8, bits)} - {thma - thmi} / 255 * {limitExpr}')
    ...
    light = edges.akarin.Expr(expr=f'x {scale_value(thlimi, 8, bits)} - {thlima - thlimi} / 255 * {limitExpr}')
    ...
    shrink = shrink.akarin.Expr(expr=f'x 4 * {limitExpr}')
    ...
    shrink = shrink.std.Convolution(matrix=rg20).std.Convolution(matrix=rg20)
    ...
    outside = core.akarin.Expr([large, shr_med], expr=f'x y - 2 * {limitExpr}')

    # If edge processing is required, adds the edgemask
    if edgeproc > 0:
        outside = core.akarin.Expr([outside, strong], expr=f'x y {edgeproc * 0.66} * + {limitExpr}')

    # Smooth again and amplify to grow the mask a bit, otherwise the halo parts sticking to the edges could be missed
    outside = outside.std.Convolution(matrix=rg20).akarin.Expr(expr=f'x 2 * {limitExpr}')
...
def FineDehalo_contrasharp(dehaloed: vs.VideoNode, src: vs.VideoNode, level: float) -> vs.VideoNode:
    ...
    if dehaloed.format.color_family != vs.GRAY:
        ...
        dehaloed = dehaloed.std.ShufflePlanes(0, vs.GRAY)
        src = src.std.ShufflePlanes(0, vs.GRAY)
    else:
        dehaloed_orig = None

    bb = dehaloed.std.Convolution(matrix=rg11)
    bb2 = RgTools(bb).Repair(RgTools(bb).Repair(RgTools(bb).RemoveGrain(4, 2), 1), 1)
    xd = core.std.MakeDiff(bb, bb2)
    xd = xd.akarin.Expr(expr=[f'x {neutral} - 2.49 * {level} * {neutral} +'])
    xdd = core.akarin.Expr([xd, core.std.MakeDiff(src, dehaloed)], expr=[f'x {neutral} - y {neutral} - * 0 < {neutral} x {neutral} - abs y {neutral} - abs < x y ? ?'])
...
def FineDehalo2(src: vs.VideoNode, hconv: Sequence[int] = [-1, -2, 0, 0, 40, 0, 0, -2, -1], vconv: Sequence[int] = [-2, -1, 0, 0, 40, 0, 0, -1, -2], showmask: bool = False)-> vs.VideoNode:
    '''
    Try to remove 2nd order halos.
    Parameters:
        src: Clip to process.
        ...
        showmask: Shows mask.
    '''
    def grow_mask(mask: vs.VideoNode, coordinates: Sequence[int], limitExpr: str) -> vs.VideoNode:
        ...
        mask = core.akarin.Expr([mask_2, mask_1], expr=['x y -'])
        return mask.std.Convolution(matrix=rg11).akarin.Expr(expr=f'x 1.8 * {limitExpr}')
    ...
    limitExpr = '0 max 1 min' if src.format.sample_type == vs.FLOAT else ''
    if src.format.color_family != vs.GRAY:
        src_orig = src
        src = src.std.ShufflePlanes(0, vs.GRAY)
    else:
    ...
    temp_h = core.akarin.Expr([mask_h, mask_v], expr=f'x 3 * y - {limitExpr}')
    temp_v = core.akarin.Expr([mask_v, mask_h], expr=f'x 3 * y - {limitExpr}')
    mask_h = grow_mask(temp_h, [0, 1, 0, 0, 0, 0, 1, 0], limitExpr)
    mask_v = grow_mask(temp_v, [0, 0, 0, 1, 1, 0, 0, 0], limitExpr)

    if not showmask:
        last = core.std.MaskedMerge(src, fix_h, mask_h)
        last = core.std.MaskedMerge(last, fix_v, mask_v)
    else:
        last = core.akarin.Expr([mask_h, mask_v], expr=['x y max'])
...
# Holy rename argus p to smoothed, add nrmode=0 and DFTTest-related parameters
def HQDeringmod(input: vs.VideoNode, p: Optional[vs.VideoNode] = None, ringmask: Optional[vs.VideoNode] = None, mrad: int = 1, msmooth: int = 1, incedge: bool = False, mthr: int = 60, minp: int = 1, nrmode: Optional[int] = None, sigma: float = 128.0,
 sigma2: Optional[float] = None, sbsize: Optional[int] = None, sosize: Optional[int] = None, sharp: int = 1, drrep: Optional[int] = None, thr: float = 12.0, elast: float = 2.0, darkthr: Optional[float] = None, planes: Union[int, Sequence[int]] = 0, show: bool = False, backend: str = "CPU") -> vs.VideoNode:
    '''
    ...
    Parameters:
        ...
        nrmode: Kernel of deringing.
            0 = DFTTest; 1 = MinBlur(r=1); 2 = MinBlur(r=2); 3 = MinBlur(r=3)
        ...
        backend: Backend for DFTTest (DFTTest2)
    '''
    if not isinstance(input, vs.VideoNode):
    ...
    if p is None:
        if nrmode <= 0:
            p = DFTTestSelect(input, sbsize=sbsize, sosize=sosize, tbsize=1, slocation=[0.0, sigma2, 0.05, sigma, 0.5, sigma, 0.75, sigma2, 1.0, 0.0], planes=planes, backend=backend)
        else:
    ...
    if sharp <= 0:
        ...
        pre = RgTools(p, planes).RemoveGrain(4)
        if sharp == 1: mode = 12
        elif sharp == 2: mode = 2.25
        else: mode = 4.0
        method = RgTools(pre, planes).RemoveGrain(mode)
        sharpdiff = core.std.MakeDiff(pre, method, planes=planes)
        allD = core.std.MakeDiff(input, p, planes=planes)
        ssDD = RgTools(sharpdiff, planes).Repair(allD, 1)
        expr = f'x {neutral} - abs y {neutral} - abs <= x y ?'
        ssDD = core.akarin.Expr([ssDD, sharpdiff], expr=[expr if i in planes else '' for i in range(input.format.num_planes)])
    ...
    if drrep <= 0:
        ...
        repclp = RgTools(input, planes).Repair(sclp, drrep)
    ...
    if ringmask is None:
        ...
        prewittm = AvsPrewitt(input, planes=[0]).akarin.Expr(expr=[expr] if isGray else [expr, ''])
        ...
        if incedge:
            ...
            ringmask = core.akarin.Expr([omask, imask], expr=[expr] if isGray else [expr, ''])
    ...
    if show:
        if isGray:
            ...
            return ringmask.akarin.Expr(expr=['', repr(neutral)])
    else:
...
def QTGMC(Input: vs.VideoNode, Preset: str = 'Slower', TR0: Optional[int] = None, TR1: Optional[int] = None, TR2: Optional[int] = None, Rep0: Optional[int] = None, Rep1: int = 0, Rep2: Optional[int] = None, EdiMode: Optional[str] = None, RepChroma: bool = True, NNSize: Optional[int] = None, NNeurons: Optional[int] = None, EdiQual: int = 1, EdiMaxD: Optional[int] = None, ChromaEdi: str = '', EdiExt: Optional[vs.VideoNode] = None, Sharpness: Optional[float] = None, SMode: Optional[int] = None, SLMode: Optional[int] = None, SLRad: Optional[int] = None, SOvs: Union[int, float] = 0, SVThin: float = 0.0, Sbb: Optional[int] = None, SrchClipPP: Optional[int] = None, SubPel: Optional[int] = None, SubPelInterp: int = 2, BlockSize: Optional[int] = None, Overlap: Optional[int] = None, Search: Optional[int] = None, SearchParam: Optional[int] = None, PelSearch: Optional[int] = None, ChromaMotion: Optional[bool] = None, TrueMotion: bool = False, Lambda: Optional[int] = None, LSAD: Optional[int] = None, PNew: Optional[int] = None, PLevel: Optional[int] = None, GlobalMotion: bool = True, DCT: int = 0, ThSAD1: int = 640, ThSAD2: int = 256, ThSCD1: int = 180, ThSCD2: int = 98, SourceMatch: int = 0, MatchPreset: Optional[str] = None, MatchEdi: Optional[str] = None, MatchPreset2: Optional[str] = None, MatchEdi2: Optional[str] = None, MatchTR2: int = 1, MatchEnhance: float = 0.5, Lossless: int = 0, NoiseProcess: Optional[int] = None, EZDenoise: Optional[float] = None, EZKeepGrain: Optional[float] = None, NoisePreset: str = 'Fast', Denoiser: Optional[str] = None, FftThreads: int = 1, DenoiseMC: Optional[bool] = None, NoiseTR: Optional[int] = None, Sigma: Optional[float] = None, ChromaNoise: bool = False, ShowNoise: Union[bool, float] = 0.0, GrainRestore: Optional[float] = None, NoiseRestore: Optional[float] = None, NoiseDeint: Optional[str] = None, StabilizeNoise: Optional[bool] = None, InputType: int = 0, ProgSADMask: Optional[float] = None, FPSDivisor: int = 1, ShutterBlur: int = 0, ShutterAngleSrc: float = 180.0, ShutterAngleOut: float = 180.0, SBlurLimit: int = 4, Border: bool = False, Precise: Optional[bool] = None, Tuning: str = 'None', ShowSettings: bool = False, GlobalNames: str = 'QTGMC', PrevGlobals: str = 'Replace', ForceTR: int = 0, Str: float = 2.0, Amp: float = 0.0625, FastMA: bool = False, ESearchP: bool = False, RefineMotion: bool = False, TFF: Optional[bool] = None, pscrn: Optional[int] = None, int16_prescreener: Optional[bool] = None, int16_predictor: Optional[bool] = None, exp: Optional[int] = None, alpha: Optional[float] = None, beta: Optional[float] = None, gamma: Optional[float] = None, nrad: Optional[int] = None, vcheck: Optional[int] = None, nnedi3_args: Mapping[str, Any] = {}, eedi3_args: Mapping[str, Any] = {}, opencl: bool = False, device: Optional[int] = None, device_type: str = "gpu", num_streams: int = 1, gauss_legacy: bool = True) -> vs.VideoNode:
    ...
    Parameters:
        ...
        SrchClipPP: Pre-filtering for motion search clip.
            0 = none; 1 = simple blur; 2 = Gauss blur; 3 = Gauss blur + edge soften
        SubPel: Sub-pixel accuracy for motion analysis.
            1 = 1 pixel; 2 = 1/2 pixel; 4 = 1/4 pixel
        SubPelInterp: Interpolation used for sub-pixel motion analysis.
            0 = bilinear (soft); 1 = bicubic (sharper); 2 = Weiner (sharpest)
        ...
        Denoiser: Select denoiser to use for noise bypass / denoising. Select from "bm3d", "dfttest", "fft3df / fft3dfilter" or "knlm / knlmeanscl / nlm".
        ...
        nnedi3_args: Additional arguments to pass to NNEDI3. e.g. pscrn, int16_prescreener, int16_predictor, exp.
        eedi3_args: Additional arguments to pass to EEDI3. e.g. alpha, beta, gamma, nrad, vcheck.
        opencl: Whether to use the OpenCL version of NNEDI3, EEDI3 and NLMeans.
        device: Sets target OpenCL or CUDA device_id, such as GPU versions of NNEDI3, EEDI3, NLMeans, BM3D and DFTTEST.
        device_type: Whether to use of NLMeans, BM3D, DFTTest:
            KNLM: cpu use nlm_ispc, gpu / cuda / cuda_rtc use nlm_cuda
            BM3D: cpu use bm3dcpu (AVX2 version), gpu / cuda use bm3dcuda, cuda_rtc use bm3dcuda_rtc
            DFTTest: cpu use DFTTest or DFTTest2 (support argus), gpu / cuda use DFTTest2 Cuda, cuda_rtc use DFTTest2 NVRTC
        num_streams: num_streams of nlm_cuda and DFTTest2 NVRTC
        gauss_legacy: true use fmtconv, false use Gauss
    '''
    ...
    if InputType != 1 and TFF is None:
        field_based = GetFrameProp(Input, '_FieldBased')
        if field_based not in [1, 2]:
    ...
    if EdiMode is not None:
        ...
        EdiMode                           = ['nnedi3', 'nnedi3', 'nnedi3', 'nnedi3', 'nnedi3', 'nnedi3', 'nnedi3', 'nnedi3', 'nnedi3', 'bwdif', 'bob'   ][pNum]
    ...
    if SourceMatch > 0:
        if MatchEdi is None:
            EdiMode = EdiMode.lower() if mpNum1 < 9 else 'bwdif'
        else:
    ...
    nnedi3_args_old = {'nsize': NNSize, 'nns': NNeurons, 'qual': EdiQual, 'pscrn': pscrn, 'int16_prescreener': int16_prescreener, 'int16_predictor': int16_predictor, 'exp': exp}
    eedi3_args_old = {'mdis': EdiMaxD, 'alpha': alpha, 'beta': beta, 'gamma': gamma, 'nrad': nrad, 'vcheck': vcheck}
    nnedi3_args = dict(**nnedi3_args_old, **nnedi3_args)
    eedi3_args  = dict(**eedi3_args_old, **eedi3_args)
    ...
    if not isinstance(srchClip, vs.VideoNode):
        ...
        if SrchClipPP == 1:
            spatialBlur = repair0.resize.Bilinear(w // 2, h // 2).std.Convolution(matrix=rg11, planes=CMplanes).resize.Bilinear(w, h)
        elif SrchClipPP >= 2:
            spatialBlur = repair0.std.Convolution(matrix=rg11, planes=CMplanes)
            spatialBlur = Resize(spatialBlur, w, h, sw=w + epsilon, sh=h + epsilon, kernel='gauss', a1=2, dmode=1) if gauss_legacy else Gauss(spatialBlur, p=2.35)
        if SrchClipPP > 1:
        ...
        if SrchClipPP <= 0:
            ...
            tweaked = core.akarin.Expr([repair0, bobbed], expr=[expr] if ChromaMotion or isGray else [expr, ''])
            expr = 'x {i7} + y < x {i2} + x {i7} - y > x {i2} - x 51 * y 49 * + 100 / ? ?'.format(i7=scale_value(7, 8, bits), i2=scale_value(2, 8, bits))
            srchClip = core.akarin.Expr([spatialBlur, tweaked], expr=[expr] if ChromaMotion or isGray else [expr, ''])
    ...
    recalculate_args = dict(thsad=ThSAD1 // 2, blksize=max(BlockSize // 2, 4), search=Search, searchparam=SearchParam, chroma=ChromaMotion, truemotion=TrueMotion, pnew=PNew, overlap=max(Overlap // 2, 2), dct=DCT)
    ...
    if NoiseProcess > 0:
        ...
        if Denoiser == 'bm3d':
            dnWindow = BM3D(noiseWindow, radius=NoiseTR, sigma=[Sigma if plane in CNplanes else 0 for plane in range(3)], device_type=device_type, device_id=device)
        elif Denoiser == 'dfttest':
            dnWindow = DFTTestSelect(noiseWindow, sigma=Sigma * 4, tbsize=noiseTD, planes=CNplanes, backend=device_type, num_streams=num_streams)
        elif Denoiser in ['knlm', 'knlmeanscl', 'nlm']:
            device_type = "opencl" if opencl and device_type is None else device_type
            dnWindow = NLMeans(noiseWindow, d=NoiseTR, h=Sigma, device_type=device_type, device_id=device, num_streams=num_streams)
        else:
    ...
    if totalRestore > 0:
        # Get actual noise from difference. Then 'deinterlace' where we have weaved noise - create the missing lines of noise in various ways
            ...
            if StabilizeNoise:
                ...
                finalNoise = core.akarin.Expr([deintNoise, mcNoise], expr=expr if ChromaNoise or isGray else [expr, ''])
            else:
    ...
    if EdiExt is not None:
        ...
        edi1 = QTGMC_Interpolate(ediInput, InputType, EdiMode, bobbed, ChromaEdi, TFF, nnedi3_args, eedi3_args, opencl, device)
    ...
    if temporalSL:
        ...
        tMax = core.akarin.Expr([core.akarin.Expr([edi, fComp1], expr=['x y max']), bComp1], expr=['x y max'])
        tMin = core.akarin.Expr([core.akarin.Expr([edi, fComp1], expr=['x y min']), bComp1], expr=['x y min'])
        if SLRad > 1:
            ...
            tMax = core.akarin.Expr([core.akarin.Expr([tMax, fComp3], expr=['x y max']), bComp3], expr=['x y max'])
            tMin = core.akarin.Expr([core.akarin.Expr([tMin, fComp3], expr=['x y min']), bComp3], expr=['x y min'])
    ...
    if SMode <= 0:
        ...
        resharp = core.akarin.Expr([lossed1, lossed1.std.Convolution(matrix=rg11)], expr=[f'x x y - {sharpAdj} * +'])
    else:
        ...
        if Precise: # Precise mode: reduce tiny overshoot
            vresharp = core.akarin.Expr([vresharp1, lossed1], expr=['x y < x {i1} + x y > x {i1} - x ? ?'.format(i1=scale_value(1, 8, bits))])
        else:
            vresharp = vresharp1
        resharp = core.akarin.Expr([lossed1, vresharp.std.Convolution(matrix=rg11)], expr=[f'x x y - {sharpAdj} * +'])
    ...
    if SVThin > 0:
        ...
        vertMedD = core.akarin.Expr([lossed1, RgTools(lossed1, 0 if isGray else None).VerticalCleaner(1)], expr=[expr] if isGray else [expr, ''])
        vertMedD = vertMedD.std.Convolution(matrix=[1, 2, 1], planes=[0], mode='h')
        expr = f'y {neutral} - abs x {neutral} - abs > y {neutral} ?'
        neighborD = core.akarin.Expr([vertMedD, vertMedD.std.Convolution(matrix=rg11, planes=[0])], expr=[expr] if isGray else [expr, ''])
    ...
    if Sbb not in [1, 3]:
        ...
        thin_gauss = core.std.MakeDiff(thin, lossed1, planes=[0]).std.Convolution(matrix=rg11, planes=[0])
        thin_gauss = Resize(thin_gauss, w, h, sw=w + epsilon, sh=h + epsilon, kernel='gauss', a1=5, dmode=1) if gauss_legacy else Gauss(thin_gauss, p=5)
        backBlend1 = core.std.MakeDiff(thin, thin_gauss, planes=[0])
    ...
    if SLMode == 1:
        if SLRad <= 1:
            sharpLimit1 = RgTools(backBlend1).Repair(edi, 1)
        else:
            sharpLimit1 = RgTools(backBlend1).Repair(RgTools(backBlend1).Repair(edi, 12), 1)
    elif SLMode == 2:
    ...
    if Sbb < 2:
        ...
        sharpLimit1_gauss = core.std.MakeDiff(sharpLimit1, lossed1, planes=0).std.Convolution(matrix=rg11, planes=0)
        sharpLimit1_gauss = Resize(sharpLimit1_gauss, w, h, sw=w + epsilon, sh=h + epsilon, kernel='gauss', a1=5, dmode=1) if gauss_legacy else Gauss(sharpLimit1_gauss, p=5)
        backBlend2 = core.std.MakeDiff(sharpLimit1, sharpLimit1_gauss, planes=0)
    ...
    if GrainRestore <= 0:
        ...
        addNoise1 = core.std.MergeDiff(backBlend2, finalNoise.akarin.Expr(expr=[expr] if ChromaNoise or isGray else [expr, '']), planes=CNplanes)
    ...
    if SLMode == 3:
        if SLRad <= 1:
            sharpLimit2 = RgTools(repair2).Repair(edi, 1)
        else:
            sharpLimit2 = RgTools(repair2).Repair(RgTools(repair2).Repair(edi, 12), 1)
    elif SLMode >= 4:
    ...
    if NoiseRestore <= 0:
        ...
        addNoise2 = core.std.MergeDiff(lossed2, finalNoise.akarin.Expr(expr=[expr] if ChromaNoise or isGray else [expr, '']), planes=CNplanes)
    ...
    if ShowNoise <= 0:
        ...
        output = finalNoise.akarin.Expr(expr=[expr] if ChromaNoise or isGray else [expr, repr(neutral)])
...
def QTGMC_Interpolate(Input: vs.VideoNode, InputType: int, EdiMode: str, Fallback: Optional[vs.VideoNode] = None, ChromaEdi: str = '', TFF: Optional[bool] = None, nnedi3_args: Mapping[str, Any] = {}, eedi3_args: Mapping[str, Any] = {}, opencl: bool = False, device: Optional[int] = None) -> vs.VideoNode:
    '''
    ...
    nnedi3_args = {'field': field, 'planes': planes, **nnedi3_args}
    nnedi3_args_c = {'planes': [1, 2], 'nsize': 4, 'nns': 0, 'qual': 1}
    eedi3_args = {'field': field, 'planes': planes, **eedi3_args}

    if opencl:
        nnedi3_args_cl = {key:val for key, val in nnedi3_args.items() if key not in ['int16_prescreener', 'int16_predictor', 'exp']}
        nnedi3_args_cl_c = {**nnedi3_args_cl, **nnedi3_args_c}
        nnedi3 = partial(core.nnedi3cl.NNEDI3CL, device=device, **nnedi3_args_cl)
        nnedi3c = partial(core.nnedi3cl.NNEDI3CL, device=device, **nnedi3_args_cl_c)
        eedi3 = partial(core.eedi3m.EEDI3CL, device=device, **eedi3_args)
    else:
        nnedi3_args_c = {**nnedi3_args, **nnedi3_args_c}
        nnedi3 = partial(core.znedi3.nnedi3, **nnedi3_args)
        nnedi3c = partial(core.znedi3.nnedi3, **nnedi3_args_c)
        eedi3 = partial(core.eedi3m.EEDI3, **eedi3_args)
    ...
    if InputType == 1:
        ...
        interp = eedi3(Input, sclip=nnedi3(Input))
    elif EdiMode == 'eedi3':
        interp = eedi3(Input)
    elif EdiMode == 'bwdif':
...
def QTGMC_KeepOnlyBobShimmerFixes(Input: vs.VideoNode, Ref: vs.VideoNode, Rep: int = 1, Chroma: bool = True) -> vs.VideoNode:
    ...
    choke1 = diff.std.Minimum(planes=planes, coordinates=coordinates)              #      x x x x x x x x    1 pixel   \
    if ed > 2: choke1 = choke1.std.Minimum(planes=planes, coordinates=coordinates) #      . . . x x x x x    1 pixel    |  Deflate to remove thin areas
    if ed > 5: choke1 = choke1.std.Minimum(planes=planes, coordinates=coordinates) #      . . . . . . x x    1 pixel   /
    if ed % 3 != 0: choke1 = choke1.std.Deflate(planes=planes)                     #      . x x . x x . x    A bit more deflate & some horizonal effect
    if ed in [2, 5]: choke1 = RgTools(choke1, planes).RemoveGrain(4)               #      . . x . . x . .    Local median
    choke1 = choke1.std.Maximum(planes=planes, coordinates=coordinates)            #      x x x x x x x x    1 pixel  \
    if ed > 1: choke1 = choke1.std.Maximum(planes=planes, coordinates=coordinates) #      . . x x x x x x    1 pixel   | Reflate again
    if ed > 4: choke1 = choke1.std.Maximum(planes=planes, coordinates=coordinates) #      . . . . . x x x    1 pixel  /
    ...
    if ed > 2: choke2 = choke2.std.Maximum(planes=planes, coordinates=coordinates)
    if ed > 5: choke2 = choke2.std.Maximum(planes=planes, coordinates=coordinates)
    if ed % 3 != 0: choke2 = choke2.std.Inflate(planes=planes)
    if ed in [2, 5]: choke2 = RgTools(choke2, planes).RemoveGrain(4)
    choke2 = choke2.std.Minimum(planes=planes, coordinates=coordinates)
    if ed > 1: choke2 = choke2.std.Minimum(planes=planes, coordinates=coordinates)
    if ed > 4: choke2 = choke2.std.Minimum(planes=planes, coordinates=coordinates)
    ...
    restore = core.akarin.Expr([core.akarin.Expr([diff, choke1], expr=[expr1] if Chroma or isGray else [expr1, '']), choke2], expr=[expr2] if Chroma or isGray else [expr2, ''])
...
def QTGMC_Generate2ndFieldNoise(Input: vs.VideoNode, InterleavedClip: vs.VideoNode, ChromaNoise: bool = False, TFF: Optional[bool] = None) -> vs.VideoNode:
    ...
    varRandom = core.akarin.Expr([core.std.MakeDiff(noiseMax, noiseMin, planes=planes), random], expr=[expr] if ChromaNoise or isGray else [expr, ''])
...
def QTGMC_MakeLossless(Input: vs.VideoNode, Source: vs.VideoNode, InputType: int, TFF: Optional[bool] = None) -> vs.VideoNode:
    ...
    vertMedian = RgTools(processed).VerticalCleaner(1)
    ...
    vmNewDiff2 = core.akarin.Expr([RgTools(vmNewDiff1).VerticalCleaner(1), vmNewDiff1], expr=[expr])
    vmNewDiff3 = RgTools(vmNewDiff2).Repair(RgTools(vmNewDiff2).RemoveGrain(2), 1)
...
def QTGMC_ApplySourceMatch(Deinterlace: vs.VideoNode, InputType: int, Source: vs.VideoNode, bVec1: Union[vs.VideoNode, None], fVec1: Union[vs.VideoNode, None], bVec2: Union[vs.VideoNode, None], fVec2: Union[vs.VideoNode, None], SubPel: int, SubPelInterp: int, hpad: int, vpad: int, ThSAD1: int, ThSCD1: int, ThSCD2: int, SourceMatch: int, MatchTR1: int, MatchEdi: str, MatchNNSize: int, MatchNNeurons: int, MatchEdiQual: int, MatchEdiMaxD: int, MatchTR2: int, MatchEdi2: str, MatchNNSize2: int, MatchNNeurons2: int, MatchEdiQual2: int, MatchEdiMaxD2: int, MatchEnhance: float, TFF: Optional[bool] = None, nnedi3_args: dict[str, Any] = {}, eedi3_args: dict[str, Any] = {}, opencl: bool = False, device: Optional[int] = None) -> vs.VideoNode:
    '''
    ...
    nnedi3_args_Interpolate = nnedi3_args.copy()
    nnedi3_args_Interpolate.update({'nsize': MatchNNSize, 'nns': MatchNNeurons, 'qual': MatchEdiQual})
    eedi3_args_Interpolate = eedi3_args.copy()
    eedi3_args_Interpolate.update({'mdis': MatchEdiMaxD})
    nnedi3_args_Interpolate2 = nnedi3_args.copy()
    nnedi3_args_Interpolate2.update({'nsize': MatchNNSize2, 'nns': MatchNNeurons2, 'qual': MatchEdiQual2})
    eedi3_args_Interpolate2 = eedi3_args.copy()
    eedi3_args_Interpolate2.update({'mdis': MatchEdiMaxD2})
    ...
    if SourceMatch < 1 or MatchTR1 <= 0:
        ...
        match1Update = core.akarin.Expr([Source, match1Clip], expr=[f'x {errorAdjust1 + 1} * y {errorAdjust1} * -'])
    if SourceMatch > 0:
        match1Edi = QTGMC_Interpolate(match1Update, InputType, MatchEdi, TFF=TFF, nnedi3_args=nnedi3_args_Interpolate, eedi3_args=eedi3_args_Interpolate, opencl=opencl, device=device)
        if MatchTR1 > 0:
    ...
    if SourceMatch > 1 and MatchEnhance > 0:
        match1Shp = core.akarin.Expr([match1, match1.std.Convolution(matrix=rg11)], expr=[f'x x y - {MatchEnhance} * +'])
    else:
        match1Shp = match1
    ...
    if SourceMatch > 1:
        ...
        match2Edi = QTGMC_Interpolate(match2Diff, InputType, MatchEdi2, TFF=TFF, nnedi3_args=nnedi3_args_Interpolate2, eedi3_args=eedi3_args_Interpolate2, opencl=opencl, device=device)
        if MatchTR2 > 0:
    ...
    if SourceMatch < 3 or MatchTR2 <= 0:
        ...
        match3Update = core.akarin.Expr([match2Edi, match2], expr=[f'x {errorAdjust2 + 1} * y {errorAdjust2} * -'])
    if SourceMatch > 2:
        if MatchTR2 > 0:
...
def smartfademod(clip: vs.VideoNode, threshold: float = 0.4, show: bool = False, tff: Optional[bool] = None) -> vs.VideoNode:
    ...
    Parameters:
        ...
        tff: Since VapourSynth only has a weak notion of field order internally, tff may have to be set. Setting tff to true means top field first and false
            means bottom field first. Note that the _FieldBased frame property, if present, takes precedence over tff.
    '''
    def frame_eval(n, f: Sequence[vs.VideoFrame], orig: vs.VideoNode, defade: vs.VideoNode) -> vs.VideoNode:
    ...
    if tff is None:
        if GetFrameProp(clip, '_FieldBased') not in [1, 2]:
...
def srestore(source: vs.VideoNode, frate: Optional[numbers.Real] = None, omode: int=6, speed: Optional[int]=None, mode: int=2, thresh: int=16, dclip: Optional[vs.VideoNode]=None) -> vs.VideoNode:
    return muf.srestore(source, frate, omode, speed, mode, thresh, dclip)

# frame_ref = start of AABCD pattern
def dec_txt60mc(src, frame_ref, srcbob=False, draft=False, tff=None, opencl=False, device=None, device_type=None) -> vs.VideoNode:
    if not isinstance(src, vs.VideoNode):
    ...
    if srcbob:
        ...
        last = QTGMC(src, TR0=1, TR1=1, TR2=1, SourceMatch=3, Lossless=2, TFF=tff, opencl=opencl, device=device, device_type=device_type)
...
# 30pテロ部を24pに変換して返す
def ivtc_txt30mc(src, frame_ref, draft=False, tff=None, opencl=False, device=None, device_type=None) -> vs.VideoNode:
    if not isinstance(src, vs.VideoNode):
...
def ivtc_txt60mc(src, frame_ref, srcbob=False, draft=False, tff=None, opencl=False, device=None, device_type=None) -> vs.VideoNode:
    if not isinstance(src, vs.VideoNode):
...
### ------------------
###    The same parameters of NLMeans.
###
### device_type [str, default: gpu]
### ------------------
###    Device of NLM, such as cpu/opencl/gpu/cuda
###
### +----------------+
### |  REQUIREMENTS  |
### +----------------+
###
### -> KNLMeansCL (device_type=opencl) / NLM_CUDA (device_type=gpu or cuda) / NLM_ISPC (device_type=cpu)
### -> RGVS
def logoNR(dlg, src, chroma=True, l=0, t=0, r=0, b=0, d=1, a=2, s=2, h=3, device_type:str="gpu", device_id:Optional[int]=None, num_streams: int = 1) -> vs.VideoNode:
    ...
    clp_nr = NLMeans(last, d=d, a=a, s=s, h=h, device_type=device_type, device_id=device_id, num_streams=num_streams)
    logoM = mt_expand_multi(core.akarin.Expr([last, src], expr=['x y - abs 16 *']), mode='losange', sw=3, sh=3).std.Convolution(matrix=rg19).std.Deflate()
...
def Vinverse(clp, sstr=2.7, amnt=255, chroma=True, scl=0.25) -> vs.VideoNode:
    if not isinstance(clp, vs.VideoNode):
    ...
    vshrp = core.akarin.Expr([vblur, vblur.std.Convolution(matrix=[1, 4, 6, 4, 1], mode='v')], expr=[f'x x y - {sstr} * +'])
    ...
    vlimD = core.akarin.Expr([vshrpD, vblurD], expr=[expr])
    last = core.std.MergeDiff(vblur, vlimD)
    if amnt <= 0:
        return clp
    elif amnt < 255:
        last = core.akarin.Expr([clp, last], expr=['x {AMN} + y < x {AMN} + x {AMN} - y > x {AMN} - y ? ?'.format(AMN=scale(amnt, peak))])
...
def Vinverse2(clp, sstr=2.7, amnt=255, chroma=True, scl=0.25) -> vs.VideoNode:
    if not isinstance(clp, vs.VideoNode):
    ...
    vblur = sbrV(clp)
    vblurD = core.std.MakeDiff(clp, vblur)
    vshrp = core.akarin.Expr([vblur, vblur.std.Convolution(matrix=[1, 2, 1], mode='v')], expr=[f'x x y - {sstr} * +'])
    vshrpD = core.std.MakeDiff(vshrp, vblur)
    expr = f'x {neutral} - y {neutral} - * 0 < x {neutral} - abs y {neutral} - abs < x y ? {neutral} - {scl} * {neutral} + x {neutral} - abs y {neutral} - abs < x y ? ?'
    vlimD = core.akarin.Expr([vshrpD, vblurD], expr=[expr])
    last = core.std.MergeDiff(vblur, vlimD)
...
#
###################
def LUTDeCrawl(input, ythresh=10, cthresh=10, maxdiff=50, scnchg=25, usemaxdiff=True, mask=False) -> vs.VideoNode:
    ...
    average_y = core.akarin.Expr([input_minus_y, input_plus_y], expr=[f'x y - abs {ythresh} < x y + 2 / 0 ?'])
    average_u = core.akarin.Expr([input_minus_u, input_plus_u], expr=[f'x y - abs {cthresh} < {peak} 0 ?'])
    average_v = core.akarin.Expr([input_minus_v, input_plus_v], expr=[f'x y - abs {cthresh} < {peak} 0 ?'])

    ymask = average_y.std.Binarize(threshold=1 << shift)
    if usemaxdiff:
        diffplus_y = core.akarin.Expr([input_plus_y, input_y], expr=[f'x y - abs {maxdiff} < {peak} 0 ?'])
        diffminus_y = core.akarin.Expr([input_minus_y, input_y], expr=[f'x y - abs {maxdiff} < {peak} 0 ?'])
...
#
###################
def LUTDeRainbow(input:vs.VideoNode, cthresh:int=10, ythresh:int=10, y:bool=True, linkUV:bool=True, mask:bool=False) -> vs.VideoNode:

    sDepth = input.format.bits_per_sample

    if not isinstance(input, vs.VideoNode) or input.format.color_family != vs.YUV or sDepth > 16:
    ...
    input_u = input.std.ShufflePlanes(1, vs.GRAY)
    input_v = input.std.ShufflePlanes(2, vs.GRAY)
    input_minus_yuv = SplitPlanes(input_minus)
    input_plus_yuv = SplitPlanes(input_plus)

    average_y = core.akarin.Expr([input_minus_yuv[0], input_plus_yuv[0]], expr=[f'x y - abs {ythresh} < {peak} 0 ?']).resize.Bilinear(input_u.width, input_u.height)
    average_u = core.akarin.Expr([input_minus_yuv[1], input_plus_yuv[1]], expr=[f'x y - abs {cthresh} < x y + 2 / 0 ?'])
    average_v = core.akarin.Expr([input_minus_yuv[2], input_plus_yuv[2]], expr=[f'x y - abs {cthresh} < x y + 2 / 0 ?'])

    umask = average_u.std.Binarize(threshold=21 << shift)
    vmask = average_v.std.Binarize(threshold=21 << shift)

    if useExpr:
        themask = core.akarin.Expr([umask, vmask], expr=[f'x y + {peak + 1} < 0 {peak} ?'])
        if y:
...
##############################################################################
def Stab(clp, dxmax=4, dymax=4, mirror=0) -> vs.VideoNode:
    if not isinstance(clp, vs.VideoNode):
        raise vs.Error('Stab: this is not a clip')

    temp = AverageFrames(clp, weights=[1] * 15, scenechange=25 / 255)
    inter = core.std.Interleave([RgTools(temp).Repair(AverageFrames(clp, weights=[1] * 3, scenechange=25 / 255), 1), clp])
...
###
######
def GSMC(input, p=None, Lmask=None, nrmode=None, radius=1, adapt=-1, rep=13, planes=None, thSAD=300, thSADC=None, thSCD1=300, thSCD2=100, limit=None, limitc=None, auto_pad:bool=False) -> vs.VideoNode:
    if not isinstance(input, vs.VideoNode):
    ...
    hpad, vpad = 0, 0
    if auto_pad:
        hpad = vpad = blksize - overlap
        clip = Padding(input, right=hpad, bottom=vpad)
        if p is not None:
            p = Padding(p, right=hpad, bottom=vpad)
        if Lmask is not None:
            Lmask = Padding(Lmask, right=hpad, bottom=vpad)
    else:
        clip = input
    ...
    if p is not None:
        ...
        pre_nr = clip.std.Convolution(matrix=rg20, planes=planes)
    else:
        pre_nr = sbr(clip, r=nrmode, planes=planes)
    dif_nr = core.std.MakeDiff(clip, pre_nr, planes=planes)
    ...
    if rep > 0:
        stable = RgTools(stable).Repair(clip, rep)

    if Lmask is not None:
        last = core.std.MaskedMerge(clip, stable, Lmask, planes=planes)
    elif adapt <= -1:
        last = stable
    else:
        clip_y = mvf.GetPlane(clip, 0)
        if adapt == 0:
            Lmask = clip_y.std.Convolution(matrix=rg19)
        elif adapt >= 255:
            Lmask = clip_y.std.Invert().std.Convolution(matrix=rg19)
        else:
            expr = 'x {adapt} - abs {peak} * {adapt} {neutral} - abs {neutral} + /'.format(adapt=scale(adapt, peak), peak=peak, neutral=neutral)
            Lmask = clip_y.akarin.Expr(expr=[expr]).std.Convolution(matrix=rg19)
        last = core.std.MaskedMerge(clip, stable, Lmask, planes=planes)
    
    if auto_pad:
        last = last.std.Crop(right=hpad, bottom=vpad)
    
    return last

GrainStabilizeMC = GSMC
...
def MCTemporalDenoise(i, radius=None, pfMode=3, sigma=None, twopass=None, useTTmpSm=False, limit=None, limit2=None, post=0, chroma=None, refine=False, deblock=False, useQED=None, quant1=None,
                      ...
                      settings='low', backend='cpu') -> vs.VideoNode:
    ...
    if p is not None:
        ...
        p = DFTTestSelect(i, tbsize=1, slocation=[0.0,4.0, 0.2,9.0, 1.0,15.0], planes=planes, backend=backend)
    else:
    ...
    if limit <= -1:
        ...
        expr = f'x {neutral} - abs y {neutral} - abs < x y ?'
        DD = core.akarin.Expr([pD, smD], expr=[expr] if chroma or isGray else [expr, ''])
        smL = core.std.MakeDiff(i, DD, planes=planes)
    elif limit > 0:
        expr = f'x y - abs {limit} <= x x y - 0 < y {limit} - y {limit} + ? ?'
        smL = core.akarin.Expr([sm, i], expr=[expr] if chroma or isGray else [expr, ''])
    else:
        smL = sm
    ...
    if edgeclean:
        ...
        mD = core.akarin.Expr([mS, mP.std.Inflate()], expr=[f'x y - {ECthr} <= 0 x y - ?']).std.Inflate().std.Convolution(matrix=rg20)
        smP = core.std.MaskedMerge(smP, DeHalo_alpha(DFTTestSelect(smP, tbsize=1, planes=planes, backend=backend), darkstr=0), mD, planes=planes)
    ...
    if stabilize:
        ...
        # mF = core.akarin.Expr([mM, mE], expr=['x y max']).std.Convolution(matrix=rg20)
        mF = mE.std.Convolution(matrix=rg20)
...
def SMDegrain(input, tr=2, thSAD=300, thSADC=None, RefineMotion=False, contrasharp=None, CClip=None, interlaced=False, tff=None, plane=4, Globals=0, pel=None, subpixel=2, prefilter=-1, mfilter=None,
              blksize=None, overlap=None, search=4, truemotion=None, MVglobal=None, dct=0, limit=255, limitc=None, thSCD1=400, thSCD2=130, chroma=True, hpad=None, vpad=None, Str=1.0, Amp=0.0625, device_type: str = "gpu", device_id: Optional[int] = None, num_streams: int = 1) -> vs.VideoNode:
    if not isinstance(input, vs.VideoNode):
    ...
    if not GlobalR:
        if preclip:
            ...
            pref = core.std.MaskedMerge(
                DFTTestSelect(inputP, tbsize=1, slocation=[0.0,4.0, 0.2,9.0, 1.0,15.0], planes=planes, backend=device_type, num_streams=num_streams),
                inputP,
                inputP.std.ShufflePlanes(0, vs.GRAY).akarin.Expr(expr=[expr]),
                planes=planes)
        elif prefilter >= 4:
            pref = NLMeans(inputP, d=1, a=1, h=7, device_type=device_type, device_id=device_id, num_streams=num_streams)
        else:
...
def STPresso(clp: vs.VideoNode, limit: int = 3, bias: int = 24, RGmode: Union[int, vs.VideoNode] = 4, tthr: int = 12, tlimit: int = 3, tbias: int = 49, back: int = 1, planes: PlanesType = None) -> vs.VideoNode:
    """
    ...
    if isinstance(RGmode, vs.VideoNode):
        ...
        bzz = RgTools(clp, planes).RemoveGrain(RGmode)

    last = core.akarin.Expr([clp, bzz], expr=[expr if i in planes else '' for i in plane_range])
    ...
    if back > 0:
        ...
        last = core.akarin.Expr([last, clp], expr=[expr if i in planes else '' for i in plane_range])
    return last


# a.k.a. BalanceBordersMod
def bbmod(c, cTop, cBottom, cLeft, cRight, thresh=128, blur=999) -> vs.VideoNode:
    if not isinstance(c, vs.VideoNode):
    ...
    def btb(c, cTop):
        ...
        referenceBlurChroma = BicubicResize(BicubicResize(last.akarin.Expr(expr=[f'x {neutral} - abs 2 *', '']), blurWidth * 2, cTop * 2), cWidth * 2, cTop * 2)
        ...
        originalBlurChroma = BicubicResize(BicubicResize(last.akarin.Expr(expr=[f'x {neutral} - abs 2 *', '']), blurWidth * 2, cTop * 2), cWidth * 2, cTop * 2)
        originalBlur = BicubicResize(BicubicResize(last, blurWidth * 2, cTop * 2), cWidth * 2, cTop * 2)

        balancedChroma = core.akarin.Expr([original, originalBlurChroma, referenceBlurChroma], expr=['', f'z y / 8 min 0.4 max x {neutral} - * {neutral} +'])
        expr = 'z {i} - y {i} - / 8 min 0.4 max x {i} - * {i} +'.format(i=scale(16, peak))
        balancedLuma = core.akarin.Expr([balancedChroma, originalBlur, referenceBlur], expr=[expr, 'z y - x +'])

        difference = core.std.MakeDiff(balancedLuma, original)
        difference = difference.akarin.Expr(expr=[f'x {scale(128 + thresh, peak)} min {scale(128 - thresh, peak)} max'])
...
def SigmoidInverse(src: vs.VideoNode, thr: float = 0.5, cont: float = 6.5, planes: PlanesType = [0, 1, 2]) -> vs.VideoNode:
    ...
    return src.akarin.Expr(expr=[expr if i in planes else '' for i in range(src.format.num_planes)])

# Convert back a clip to linear luminance
def SigmoidDirect(src: vs.VideoNode, thr: float = 0.5, cont: float = 6.5, planes: PlanesType = [0, 1, 2]) -> vs.VideoNode:
    if not isinstance(src, vs.VideoNode) or src.format.bits_per_sample != 16:
...
def GrainFactory3(clp, g1str=7.0, g2str=5.0, g3str=3.0, g1shrp=60, g2shrp=66, g3shrp=80, g1size=1.5, g2size=1.2, g3size=0.9, temp_avg=0, ontop_grain=0.0, seed=-1, th1=24, th2=56, th3=128, th4=160) -> vs.VideoNode:
    if not isinstance(clp, vs.VideoNode):
    ...
    grainlayer = core.std.MaskedMerge(core.std.MaskedMerge(grainlayer1, grainlayer2, clp.akarin.Expr(expr=[expr1])), grainlayer3, clp.akarin.Expr(expr=[expr2]))
...
#------------------------------------------------------------------------------#
def InterFrame(Input, Preset='Medium', Tuning='Film', NewNum=None, NewDen=1, GPU=False, InputType='2D', OverrideAlgo=None, OverrideArea=None, FrameDouble=False) -> vs.VideoNode:
    if not isinstance(Input, vs.VideoNode):
...
def FixColumnBrightness(c, column, input_low, input_high, output_low, output_high) -> vs.VideoNode:
    if not isinstance(c, vs.VideoNode):
...
def FixRowBrightness(c, row, input_low, input_high, output_low, output_high) -> vs.VideoNode:
    if not isinstance(c, vs.VideoNode):
...
def FixColumnBrightnessProtect(c, column, input_low, input_high, output_low, output_high, protect_value=20) -> vs.VideoNode:
    if not isinstance(c, vs.VideoNode):
...
def FixRowBrightnessProtect(c, row, input_low, input_high, output_low, output_high, protect_value=20) -> vs.VideoNode:
    if not isinstance(c, vs.VideoNode):
...
def FixColumnBrightnessProtect2(c, column, adj_val, prot_val=16) -> vs.VideoNode:
    if not isinstance(c, vs.VideoNode):
...
def FixRowBrightnessProtect2(c, row, adj_val, prot_val=16) -> vs.VideoNode:
    if not isinstance(c, vs.VideoNode):
...
def SmoothLevels(input, input_low=0, gamma=1.0, input_high=None, output_low=0, output_high=None, chroma=50, limiter=0, Lmode=0, DarkSTR=100, BrightSTR=100, Ecenter=None, protect=-1, Ecurve=0,
                 Smode=-2, Mfactor=2, RGmode=12, useDB=False) -> vs.VideoNode:
    if not isinstance(input, vs.VideoNode):
    ...
    if limiter == 1 or limiter >= 3:
        limitI = input.akarin.Expr(expr=[f'x {input_low} max {input_high} min'])
    else:
        limitI = input

    expr = exprL + ' ' + exprP + ' * ' + exprY + ' x - * x +'
    level = limitI.akarin.Expr(expr=[expr] if chroma <= 0 or isGray else [expr, exprC])
    diff = core.akarin.Expr([limitI, level], expr=[f'x y - {Mfactor} * {neutral[1]} +'])
    process = RgTools(diff).RemoveGrain(RGmode)
    if useDB:
        process = process.akarin.Expr(expr=[f'x {neutral[1]} - {Mfactor} / {neutral[1]} +']).f3kdb.Deband(grainy=0, grainc=0, output_depth=input.format.bits_per_sample)
        smth = core.std.MakeDiff(limitI, process)
    else:
        smth = core.akarin.Expr([limitI, process], expr=[f'x y {neutral[1]} - {Mfactor} / -'])

    level2 = core.akarin.Expr([limitI, diff], expr=[f'x y {neutral[1]} - {Mfactor} / -'])
    diff2 = core.akarin.Expr([level2, level], expr=[f'x y - {Mfactor} * {neutral[1]} +'])
    process2 = RgTools(diff2).RemoveGrain(RGmode)
    if useDB:
        process2 = process2.akarin.Expr(expr=[f'x {neutral[1]} - {Mfactor} / {neutral[1]} +']).f3kdb.Deband(grainy=0, grainc=0, output_depth=input.format.bits_per_sample)
        smth2 = core.std.MakeDiff(smth, process2)
    else:
        smth2 = core.akarin.Expr([smth, process2], expr=[f'x y {neutral[1]} - {Mfactor} / -'])

    mask1 = core.akarin.Expr([limitI, level], expr=[f'x y - abs {neutral[0] / Mfactor} >= {peak} 0 ?'])
    mask2 = core.akarin.Expr([limitI, level], expr=[f'x y - abs {peak / Mfactor} >= {peak} 0 ?'])
    ...
    if limiter >= 2:
        limitO = Slevel.akarin.Expr(expr=[f'x {output_low} max {output_high} min'])
    else:
        limitO = Slevel
...
def FastLineDarkenMOD(c:vs.VideoNode, strength:int=48, protection:int=5, luma_cap:int=191, threshold:int=4, thinning:int=0) -> vs.VideoNode:
    if not isinstance(c, vs.VideoNode):
    ...
    thick = core.akarin.Expr([c, exin], expr=[f'y {lum} < y {lum} ? x {thr} + > x y {lum} < y {lum} ? - 0 ? {Str} * x +'])
    if thinning <= 0:
        last = thick
    else:
        diff = core.akarin.Expr([c, exin], expr=[f'y {lum} < y {lum} ? x {thr} + > x y {lum} < y {lum} ? - 0 ? {scale(127, peak)} +'])
        linemask = diff.std.Minimum().akarin.Expr(expr=[f'x {scale(127, peak)} - {thn} * {peak} +']).std.Convolution(matrix=rg20)
        thin = core.akarin.Expr([c.std.Maximum(), diff], expr=[f'x y {scale(127, peak)} - {Str} 1 + * +'])
...
def Toon(input, str=1.0, l_thr=2, u_thr=12, blur=2, depth=32) -> vs.VideoNode:
    if not isinstance(input, vs.VideoNode):
    ...
    last = core.akarin.Expr([last, Padding(last, 6, 6, 6, 6).warp.AWarpSharp2(blur=blur, depth=depth).std.Crop(6, 6, 6, 6)], expr=['x y min'])
    expr = f'y {lthr} <= {neutral} y {uthr} >= x {uthr8} y {multiple} / - 128 * x {multiple} / y {multiple} / {lthr8} - * + {ludiff} / {multiple} * ? {neutral} - {str} * {neutral} + ?'
    last = core.std.MakeDiff(input, core.akarin.Expr([last, last.std.Maximum()], expr=[expr]))
...
def LSFmod(input, strength=None, Smode=None, Smethod=None, kernel=11, preblur=None, secure=None, source=None, Szrp=16, Spwr=None, SdmpLo=None, SdmpHi=None, Lmode=None, overshoot=None, undershoot=None,
           overshoot2=None, undershoot2=None, soft=None, soothe=None, keep=None, edgemode=0, edgemaskHQ=None, ss_x=None, ss_y=None, dest_x=None, dest_y=None, defaults='fast', backend='cpu') -> vs.VideoNode:
    if not isinstance(input, vs.VideoNode):
    ...
    if preblur <= -1:
        ...
        expr = 'x {i} < {peak} x {j} > 0 {peak} x {i} - {peak} {j} {i} - / * - ? ?'.format(i=scale(16, peak), j=scale(75, peak), peak=peak)
        pre = core.std.MaskedMerge(DFTTestSelect(tmp, tbsize=1, slocation=[0.0,4.0, 0.2,9.0, 1.0,15.0], backend=backend), tmp, tmp.akarin.Expr(expr=[expr]))
    else:
    ...
    if Smode < 3:
        if Smethod <= 1:
            method = RgTools(pre).RemoveGrain(kernel)
        elif Smethod == 2:
            method = core.std.Merge(dark_limit, bright_limit)
        else:
            method = RgTools(core.std.Merge(dark_limit, bright_limit)).RemoveGrain(kernel)

        if secure:
            method = core.akarin.Expr([method, pre], expr=['x y < x {i} + x y > x {i} - x ? ?'.format(i=scale(1, peak))])

        if preblur > -1:
            method = core.std.MakeDiff(tmp, core.std.MakeDiff(pre, method))

        if Smode <= 1:
            normsharp = core.akarin.Expr([tmp, method], expr=[f'x x y - {Str} * +'])
        else:
            tmpScaled = tmp.akarin.Expr(expr=[f'x {1 / factor if isInteger else factor} *'], format=tmp.format.replace(sample_type=vs.FLOAT, bits_per_sample=32))
            methodScaled = method.akarin.Expr(expr=[f'x {1 / factor if isInteger else factor} *'], format=method.format.replace(sample_type=vs.FLOAT, bits_per_sample=32))
            ...
            normsharp = core.akarin.Expr([tmpScaled, methodScaled], expr=[expr], format=tmp.format)
    else:
        normsharp = pre.cas.CAS(sharpness=min(Str, 1))

        if secure:
            normsharp = core.akarin.Expr([normsharp, pre], expr=['x y < x {i} + x y > x {i} - x ? ?'.format(i=scale(1, peak))])
    ...
    if edgemaskHQ:
        ...
        edge = core.akarin.Expr([tmp.std.Maximum(), tmp.std.Minimum()], expr=['x y -'])
    edge = edge.akarin.Expr(expr=[f'x {1 / factor if isInteger else factor} * {128 if edgemaskHQ else 32} / 0.86 pow 255 * {factor if isInteger else 1 / factor} *'])

    if Lmode < 0:
        limit1 = RgTools(normsharp).Repair(tmp, abs(Lmode))
    elif Lmode == 0:
        limit1 = normsharp
    elif Lmode == 1:
        limit1 = normal
    ...
    if edgemode <= 0:
        ...
        limit2 = core.std.MaskedMerge(tmp, limit1, edge.std.Inflate().std.Inflate().std.Convolution(matrix=rg11))
    else:
        limit2 = core.std.MaskedMerge(limit1, tmp, edge.std.Inflate().std.Inflate().std.Convolution(matrix=rg11))
    ...
    if soft == 0:
        ...
        sharpdiff = core.akarin.Expr([sharpdiff, sharpdiff.std.Convolution(matrix=rg19)], expr=[f'x {neutral} - abs y {neutral} - abs > y {soft} * x {100 - soft} * + 100 / x ?'])
        PP1 = core.std.MakeDiff(tmp, sharpdiff)

    ### SOOTHE
    if soothe:
        diff = core.std.MakeDiff(tmp, PP1)
        diff = core.akarin.Expr([diff, AverageFrames(diff, weights=[1] * 3, scenechange=32 / 255)],
    ...
    if edgemode <= -1:
        ...
        expr = f'x {neutral} - abs y {neutral} - abs < x y ?'
        shrpL = core.akarin.Expr([RgTools(shrpD).Repair(core.std.MakeDiff(In, src, planes=[0]), 1), shrpD], expr=[expr] if isGray else [expr, ''])
...
def TemporalDegrain(inpClip, denoiseClip = None, sigma = 16, blockWidth = 16, blockHeight = 16, sigma2 = None, sigma3 = None, sigma4 = None, overlapWidth = None, overlapHeight = None, blockSize = 16, pel = 2, overlapValue = None, degrain = 2, maxPxChange = 255, thrDegrain1 = 400, thrDegrain2 = 300, HQ = 1) -> vs.VideoNode:
    ...
    dd = core.akarin.Expr([spatD, nr1Diff], expr=[f'x {neutral} - abs y {neutral} - abs < x y ?'])
...
def aaf(inputClip: vs.VideoNode, mode: str = "repair", aas: float = -0.7, aar: Optional[float] = None, aay: int = 28, aax: Optional[int] = None, estr: int = 255, bstr: int = 40) -> vs.VideoNode:
    '''
    parameter description:
        mode:
            there are two modes you can use to reduce the side effects of sangnom;
            the default mode is "repair", it’s faster then the second mode="edge" and avoid most artefacts also for smaller aas values;
            the mode "edge" filters only on edges and keep details sharper, read also estr/bstr, if you set another string than these two, no postprocessing will be done;
        aas:
            this is the basic quality vs speed factor of aaf    ->anti aliasing scaling
            negative values process the horizontal and vertical direction without resizing the complete source, this is much faster than with the absolut value, but will also create more artefacts if you don’t use a repair mode
            that higher the absolut value of aas that higher is the scaling factor, that better is the quality, that slower the function and that lower the antialiasing effect with aas=1.0 aaf performs like aa() and aaa()
            [-2.0...2.0  -> -0.7]
        aay/aax
    ...
    if mode == "repair":
        return RgTools(aa).Repair(inputClip, 18)
    ...
    mask = mask.akarin.Expr(expr=[expr] if isGray else [expr, ''])

    merged = core.std.MaskedMerge(inputClip, aa, mask, planes=0)
    if aas > 0.84:
        return merged
    return RgTools(merged).Repair(inputClip, 18)


def AverageFrames(clip: vs.VideoNode, weights: Union[float, Sequence[float]], scenechange: Optional[float] = None, planes: PlanesType = None) -> vs.VideoNode:
    if not isinstance(clip, vs.VideoNode):
...
def AvsPrewitt(clip: vs.VideoNode, planes: PlanesType = None) -> vs.VideoNode:
    if not isinstance(clip, vs.VideoNode):
    ...
    return core.akarin.Expr([clip.std.Convolution(matrix=[1, 1, 0, 1, 0, -1, 0, -1, -1], planes=planes, saturate=False),
...
def Bob(clip: vs.VideoNode, b: float = 1 / 3, c: float = 1 / 3, tff: Optional[bool] = None) -> vs.VideoNode:
    ...
    if hasattr(core.resize, 'Bob'):
        clip = clip.resize.Bob(tff=tff, filter_param_a=b, filter_param_b=c)
    else:
...
def Gauss(clip: vs.VideoNode, p: Optional[float] = None, sigma: Optional[float] = None, planes: PlanesType = None) -> vs.VideoNode:
    if not isinstance(clip, vs.VideoNode):
    ...
    return clip.std.Convolution(matrix=kernel, planes=planes, mode='v').std.Convolution(matrix=kernel, planes=planes, mode='h')


def Clamp(clip: vs.VideoNode, bright_limit: vs.VideoNode, dark_limit: vs.VideoNode, overshoot: int = 0, undershoot: int = 0, planes: PlanesType = None) -> vs.VideoNode:
    ...
    return core.akarin.Expr([clip, bright_limit, dark_limit], expr=[f'x y {overshoot} + min z {undershoot} - max' if i in planes else '' for i in range(clip.format.num_planes)])
mt_clamp = Clamp


def NLMeans(clip: vs.VideoNode, d: Optional[int] = None, a: Optional[int] = None, s: Optional[int] = None, h: Optional[float] = None, wmode: Optional[int] = None, wref: Optional[float] = None, rclip: Optional[vs.VideoNode] = None, device_type: str = "gpu", device_id: Optional[int] = None, num_streams: int = 1) -> vs.VideoNode:

    if not isinstance(clip, vs.VideoNode):
        raise vs.Error('NLMeans: this is not a clip')

    if clip.format.color_family not in [vs.YUV, vs.GRAY]:
        raise vs.Error('NLMeans: this wrapper is intended to be used only for YUV and GREY format')

    device_type = device_type.lower()
    nlm_args = dict(d=d, a=a, s=s, h=h, wmode=wmode, wref=wref, rclip=rclip)

    if device_type == "cpu":
        NLMeans = partial(core.nlm_ispc.NLMeans, **nlm_args)
    elif device_type == "opencl":
        NLMeans = partial(core.knlm.KNLMeansCL, **nlm_args, device_type="gpu", device_id=device_id)
    elif device_type in ["gpu", "cuda", "cuda_rtc"]:
        NLMeans = partial(core.nlm_cuda.NLMeans, **nlm_args, device_id=device_id, num_streams=num_streams)
    else:
        raise vs.Error("NLMeans: device_type only support cpu, opencl, gpu, cuda and cuda_rtc.")

    if clip.format.color_family == vs.YUV:
        if clip.format.subsampling_w > 0 or clip.format.subsampling_h > 0:
            clip_hf = clip.fmtc.resample(clip.width//2, clip.height//2, kernel="bicubic", a1=0, a2=0.5, planes=[3,2,2], css="444")
            nlm_luma, nlm_c = NLMeans(clip), NLMeans(clip_hf)
            return core.std.ShufflePlanes([nlm_luma, nlm_c], [0,1,2], vs.YUV)
        else:
            return NLMeans(clip, channels="YUV")
    else:
        return NLMeans(clip)
KNLMeansCL = NLMeans


def DFTTestSelect(clip: vs.VideoNode, sigma: float = 8.0, sbsize: int = 16, sosize: int = 12, tbsize: int = 3, nlocation: Optional[Sequence[int]] = None, slocation: Optional[Union[Sequence[Tuple[float]], Sequence[float]]] = None, planes: PlanesType = None, backend: str = "CPU", num_streams: int = 1) -> vs.VideoNode:
    '''Help select DFTTest and DFTTest2'''
    
    backendL = backend.lower()
    dfttest_argus = dict(sigma=sigma, sbsize=sbsize, sosize=sosize, tbsize=tbsize, nlocation=nlocation, slocation=slocation, planes=planes)

    if backendL == "cpu":
        if sbsize == 16 and tbsize in [1, 3, 5, 7]:
            from dfttest2 import DFTTest, Backend
            res = DFTTest(clip, backend=Backend.CPU(), **dfttest_argus)
        else:
            res = clip.dfttest.DFTTest(**dfttest_argus)
    else:
        from dfttest2 import DFTTest, Backend
        
        if backendL in ["cuda", "gpu"]:
            backendT = Backend.cuFFT()
        elif backendL in ["nvrtc", "cuda_rtc"]:
            backendT = Backend.NVRTC(num_streams=num_streams)
        elif backendL == "gcc":
            backendT = Backend.GCC()
        else:
            raise ValueError("DFTTestSelect: backend support cpu, cuda, nvrtc, gcc")

        res = DFTTest(clip, backend=backendT, **dfttest_argus)
    
    return res


def Overlay(base: vs.VideoNode, overlay: vs.VideoNode, x: int = 0, y: int = 0, mask: Optional[vs.VideoNode] = None, opacity: float = 1.0, mode: str = 'normal', planes: PlanesType = None, mask_first_plane: bool = True) -> vs.VideoNode:
    '''
    ...
    if opacity < 1:
        mask = mask.akarin.Expr(expr=[f'x {opacity} *'])
    ...
    if mode != 'normal':
        overlay = core.akarin.Expr([overlay, base], expr=[expr if i in planes else '' for i in range(base.format.num_planes)])
...
def Resize(src, w, h, sx=None, sy=None, sw=None, sh=None, kernel=None, taps=None, a1=None, a2=None, invks=None, invkstaps=None, css=None, planes=None, center=None, cplace=None, cplaces=None, cplaced=None, interlaced=None, interlacedd=None, tff=None, tffd=None, flt=None, noring=False, bits=None, fulls=None, fulld=None, dmode=None, ampo=None, ampn=None, dyn=None, staticnoise=None, patsize=None) -> vs.VideoNode:
    if not isinstance(src, vs.VideoNode):
    ...
    if nrf:
        ...
        last = RgTools(main).Repair(nrng, 1)
...
def Weave(clip: vs.VideoNode, tff: Optional[bool] = None) -> vs.VideoNode:
    ...
    if tff is None:
        if GetFrameProp(clip, '_Field') not in [1, 2]:
...
# Holy change the default of rep=13 and radius=None to 1 (Orignal AVS scripts values). Only process luma plane by default for non-RGB formats
def ContraSharpening(denoised: vs.VideoNode, original: vs.VideoNode, radius: int = 1, rep: int = 1, planes: PlanesType = None) -> vs.VideoNode:
    '''
    contra-sharpening: sharpen the denoised clip, but don't add more to any pixel than what was removed previously.
    Parameters:
    ...
    if radius <= 1: rg_mode = 12
    elif radius == 2: rg_mode = 2.25
    else: rg_mode = 2.75
    
    RG11 = RgTools(s, planes).RemoveGrain(rg_mode)
    ...
    ssDD = RgTools(ssD).Repair(allD, rep)
    # abs(diff) after limiting may not be bigger than before
    ssDD = core.akarin.Expr([ssDD, ssD], expr=[f'x {neutral} - abs y {neutral} - abs < x y ?' if i in planes else '' for i in range(denoised.format.num_planes)])
...
def MinBlur(clp: vs.VideoNode, r: int = 1, planes: PlanesType = None) -> vs.VideoNode:
    ...
    if r <= 0:
        ...
        if r == 1: rg11_m, rg4_r = 11, 1
        elif r == 2: rg11_m, rg4_r = 2.25, 2
        else: rg11_m, rg4_r = 4.0, 3
        RG11 = RgTools(clp, planes).RemoveGrain(rg11_m)
    RG4 = RgTools(clp, planes).RemoveGrain(4, rg4_r)

    expr = 'x y - x z - * 0 < x x y - abs x z - abs < y z ? ?'
    return core.akarin.Expr([clp, RG11, RG4], expr=[expr if i in planes else '' for i in range(clp.format.num_planes)])


def sbr(c: vs.VideoNode, r: int = 1, planes: PlanesType = None) -> vs.VideoNode:
    ...
    RG11 = RgTools(c, planes).RemoveGrain(rmode)
    RG11D = core.std.MakeDiff(c, RG11, planes=planes)
    RG11DS = RgTools(RG11D, planes).RemoveGrain(rmode)

    expr = f'x y - x {neutral} - * 0 < {neutral} x y - abs x {neutral} - abs < x y - {neutral} + x ? ?'
    RG11DD = core.akarin.Expr([RG11D, RG11DS], expr=[expr if i in planes else '' for i in range(c.format.num_planes)])
...
def DitherLumaRebuild(src: vs.VideoNode, s0: float = 2.0, c: float = 0.0625, chroma: bool = True) -> vs.VideoNode:
    ...
    k = (s0 - 1) * c
    t = f'x {scale_value(16, 8, bits)} - {scale_value(219, 8, bits)} / 0 max 1 min' if isInteger else 'x 0 max 1 min'
    e = f'{k} {1 + c} {(1 + c) * c} {t} {c} + / - * {t} 1 {k} - * + ' + (f'{scale_value(256, 8, bits)} *' if isInteger else '')
    return src.akarin.Expr(expr=[e] if isGray else [e, f'x {neutral} - 128 * 112 / {neutral} +' if chroma and isInteger else ''])
...
#=============================================================================
def mt_expand_multi(src: vs.VideoNode, mode: str = 'rectangle', planes: PlanesType = None, sw: int = 1, sh: int = 1) -> vs.VideoNode:
    '''
...
def mt_inpand_multi(src: vs.VideoNode, mode: str = 'rectangle', planes: PlanesType = None, sw: int = 1, sh: int = 1) -> vs.VideoNode:
    '''
...
def mt_inflate_multi(src: vs.VideoNode, planes: PlanesType = None, radius: int = 1) -> vs.VideoNode:
    if not isinstance(src, vs.VideoNode):
...
def mt_deflate_multi(src: vs.VideoNode, planes: PlanesType = None, radius: int = 1) -> vs.VideoNode:
    if not isinstance(src, vs.VideoNode):
