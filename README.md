...
The Encode progress label identifies main rows as sequential `EP01`, `EP02`, and so on, and selected SP rows as sequential `SP01`, `SP02`, and so on, independently of user-editable output names or unselected rows. Every selected table3 row consumes one SP number in visible order, including copy-only image folders and extracted IGS menu assets. Blu-ray preparation SP muxing and actual video encoding display the corresponding original output name; other SP stages use the short sequential label. The progress text distinguishes video/VapourSynth metadata analysis, video encoding, audio extraction, silent/duplicate analysis, per-track audio conversion, final Matroska muxing, and final metadata verification.
...
- The generated default VPy exposes the processed `res` as output index `0` and the original `src8` as output index `1`. In the VSEdit preview window, keep synchronization by frame number and switch **Output index** between `0` and `1` to inspect the same frame before and after processing. This live preview is separate from the PNG files generated by **Output comparison images** after encoding.
