...
5. An MPLS covered completely by a main output is shown but unchecked by default when it exposes no new audio or subtitle PID. In series mode, an exact and unique episode-detail match that exposes a new PID is checked by default and linked to that episode.
...
14. Editing tracks recalculates the output name immediately. In series mode, an SP row can target an episode output only when its M2TS detail exactly and uniquely matches one episode; linked rows are renamed immediately when episode splitting changes. When several selected SP rows target the same episode, the first row in visible order supplies a repeated PID, only PIDs absent from the episode are appended, and all appended SP PIDs remain in ascending order. Movie-mode SP rows always keep independent SP output paths and are never attached to the main output. The runtime uses the exact visible output name and does not silently rename it or rediscover another file.
