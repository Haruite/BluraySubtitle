...
- **Dolby Vision enhancement layers:** Identify MEL/FEL before Dolby Vision Remux or x265 Encode; treat identification failure as FEL. Remux converts to P8.1 only when no image residuals are discarded, otherwise retaining P7. x265 may use the base layer without FEL residuals and must report that limitation in the existing completion dialog. SVT-AV1 instead reports that Dolby Vision is not retained, without distinguishing MEL/FEL. Neither limitation fails Encode or requires advance confirmation.
...
- **Encode video timing:** Preserve the source MKV presentation timestamps for both CFR and VFR, including the video start offset. Support frame-preserving VPy processing and the README prefix test with `Trim(first=0, length=N)`; a prefix uses the corresponding source timestamps and end boundary. Hardsubs use the same timeline. General frame removal/insertion, reordering, editing, speed changes, and CFR conversion are outside this contract.
