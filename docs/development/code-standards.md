...
Keep both language versions of these standards synchronized, and record confirmed rules before or with code that relies on them. Follow section 7 for documentation maintenance. Refactoring or major changes require new [English](../refactoring/refactoring-history.md) and [Simplified Chinese](../refactoring/refactoring-history.zh-Hans.md) history entries; ordinary changes need no entry. History is not a source of requirements.
...
- Keep only the main window's top language/theme/font/settings bar and bottom execution button fixed. Scroll all other content vertically and preserve readable default table heights that users can adjust.
...
- Follow the [project documentation skill](../../.agents/skills/project-docs/SKILL.md). Decide whether to update documents based on their readers, purpose, and actual changes, keeping existing English and Chinese versions synchronized. README content must help users operate the software; link to implementation explanations elsewhere. Add no text for fixes within already documented behavior or changes that only improve implementation details.
...
Keep this section limited to confirmed exceptions and product semantics that would otherwise be ambiguous. Ordinary implementation improvements need no new entry; explain algorithms and workflows in the relevant wiki or development document.

- **ISO subtitle merging:** ISO input is limited to reading BDMV playlists for subtitle merging, not preview, Remux, or Encode.
- **Dolby Vision enhancement layers:** Identify MEL/FEL before Dolby Vision Remux or x265 Encode; treat identification failure as FEL. Remux converts to P8.1 only when no image residuals are discarded, otherwise retaining P7. x265 may use the base layer without FEL residuals and must report that limitation in the completion dialog. SVT-AV1 instead reports that Dolby Vision is not retained, without distinguishing MEL/FEL. Neither limitation fails Encode or requires advance confirmation.
- **Main-playlist Remux selection:** **Edit Tracks** controls video, audio, and subtitle selection. Manually entered track-selection flags in the mux command are replaced by the captured track selection.
- **Encode video timing:** Preserve the source MKV presentation timestamps for both CFR and VFR, including the video start offset. Support frame-preserving VPy processing and the README prefix test, keeping hardsubs synchronized. General frame removal/insertion, reordering, editing, speed changes, and CFR conversion are outside this contract.
...
- **MPLS logical tracks:** At load time, build track choices only from the complete playlist's STN. Within one MPLS, the STN category and ordinal stream number identify a logical track across PlayItems; PID changes and partial presence do not create separate logical tracks. Language changes are informational; keep the first explicit non-`und` language as the default.
...
- **Transport compatibility:** Limit MPLS and M2TS PAT/PMT checks to parameters those structures expose. Do not add speculative full-payload parsing or normalization for Matroska append incompatibilities. If MKVToolNix encounters such a change, fail explicitly; do not synthesize replacement packets, silently discard another track, or promote a partial file to final output.
...
- **Audio-gap information:** Remux saves analysis results, including empty records for continuous audio. Encode prefers valid results matching the source. Without a valid record, inspect the actual timeline rather than assuming continuity. See [Remux fallback](media-pipeline-and-tool-selection.md#3-track-aligned-remux-fallback) for file and detection details.
- **Bitrate:** FDK-AAC and Opus bitrate `0` means Auto; positive values are explicit kbps targets.
- **Series-mode SP matching:** Compare complete M2TS detail: clip names, order, and time ranges. First, a non-main MPLS matching exactly one complete selected main MPLS contributes track choices to that shared main remux. Otherwise, an SP matching exactly one episode is appended only after splitting. A partial match spanning several episodes remains an ordinary SP. Movie mode uses neither attachment path. See [SP rules](../wiki/Blu-ray-Disc-Structure.md#sp-tracks-and-mpls-hidden-tracks) for default selection and deduplication.
...
- At the end of each change batch, briefly report actual changes, important behavior changes, check results, and any necessary validation still outstanding. Scale detail to the scope of the change.
