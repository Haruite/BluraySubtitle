...
剧集模式下，当 SP 区间与唯一一个主剧集的 M2TS detail 完全相同时，可以把所选 SP 音频或字幕 PID 追加到计划剧集输出。多个已选 SP 按界面顺序处理：重复 PID 由第一行提供，已存在的 PID 会跳过，附加的 SP PID 在原正片轨道之后保持升序。电影模式 SP 不进入这条附加路径。只有追加结果完成并验证后，才替换原剧集文件。
