...
有 MPLS 的“编辑轨道”行直接由首个 PlayItem 的 STN 生成并按 PID 排列，加载期间不执行 `mkvmerge --identify`，也不探测首个 M2TS。主 Remux 命令保存 `{video_opts}`、`{audio_opts}` 和 `{sub_opts}`，而不是轨道 ID。执行时只分析一次 MPLS；任一已选 PID 缺失时进入 PID 对齐回退，映射完整时才解析占位符并继续逐个检查 M2TS 映射。
