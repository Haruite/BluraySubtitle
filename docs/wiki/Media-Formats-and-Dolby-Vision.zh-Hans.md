...
3. 用受支持位深的 x265 编码处理后的基础视频；
4. 实际 x265 声明支持原生参数，且当前行已经具备 VBV 和母版显示前提时，在同一次压制中写入 RPU，否则由 `dovi_tool` 后注入；
5. 原生产物验证失败时也回退到注入，验证编码 HEVC 中存在 RPU；来源带有 HDR10+ 时也验证 HDR10+；
6. 混流最终容器，并在验证该容器时要求 Profile 8 及与 VPy 输出一致的 RPU 帧数；当前启用的 HDR10+ 和由程序自动补充的静态字段也会再次检查。
