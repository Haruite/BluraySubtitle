...
class GetnativeWorkflowTests(unittest.TestCase):
    ...
    def test_default_vpy_applies_every_detected_kernel_parameter(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            with patch.object(vpy_edit_and_preview.sys, "platform", "linux"):
            ...
            self.assertIn('rescale_mask = _rescale_protection_mask(', content)
            self.assertIn('core.std.MaskedMerge(', content)
            self.assertIn('chromaloc_in_s="left"', content)
            self.assertIn('core.placebo.Deband(', content)
            self.assertIn('denoise_strength = 0.6', content)
            self.assertIn('dehalo_strength = 0.0', content)
            self.assertIn('dering_strength = 0.0', content)
            self.assertIn('deband_strength = 0.5', content)
            self.assertIn('antialiasing_strength = 0.5', content)
            self.assertIn('_denoise_luma_protected(proc16, denoise_strength)', content)
            self.assertIn('_dehalo_luma(nr16, dehalo_strength)', content)
            self.assertIn('_dering_luma(nr16, dering_strength)', content)
            self.assertIn('if deband_strength == 0.0:', content)
            self.assertIn('if antialiasing_strength == 0.0:', content)
            self.assertIn('detail_mask = _nr_pl.std.Prewitt()', content)
            self.assertIn(
                'core.std.MaskedMerge(\n        dbed_candidate, _nr_pl, detail_mask',
                content,
            )
            self.assertIn(
                'antialiasedY = mvf.LimitFilter(aaedY, dbedY',
                content,
            )
            self.assertNotIn(
                'antialiasedY = mvf.LimitFilter(dbedY, aaedY',
                content,
            )
            self.assertIn('channels="Y"', content)
            self.assertIn('source_y.std.Prewitt()', content)
            self.assertNotIn('h=3', content)
            self.assertNotIn('neo_f3kdb', content)
            self.assertLess(
                content.index('core.std.LoadAllPlugins(PLUGIN_PATH)'),
                content.index('import mvsfunc as mvf'),
            )
            self.assertIn('os.environ.get("BLURAYSUB_PLUGIN_PATH")', content)
            self.assertIn('cachefile=_lwi', content)
            self.assertIn('props.get("_FieldBased", 0)', content)
            self.assertNotIn('ffms2', content)
    ...
    def test_existing_default_vpy_gets_the_detail_preserving_filter_upgrade(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            vpy_path = Path(temp_dir) / "vpy.vpy"
            vpy_path.write_text(
                'native_kernel = ""  # optional, auto-generated by app\n\n'
                + vpy_edit_and_preview._PREVIOUS_GETNATIVE_KERNEL_HELPER
            ...
            content = vpy_path.read_text(encoding="utf-8")
            self.assertIn(vpy_edit_and_preview._VPY_PROCESSING_SETTINGS, content)
            self.assertIn(vpy_edit_and_preview._VPY_PROCESSING_HELPERS, content)
            self.assertIn(vpy_edit_and_preview._VPY_FILTER_BLOCK, content)
            self.assertIn(vpy_edit_and_preview._GETNATIVE_KERNEL_HELPER, content)
            self.assertIn(vpy_edit_and_preview._GETNATIVE_DESCALE_BLOCK, content)
            self.assertNotIn(vpy_edit_and_preview._PREVIOUS_GETNATIVE_DESCALE_BLOCK, content)
            self.assertNotIn(vpy_edit_and_preview._LEGACY_VPY_FILTER_BLOCK, content)
    ...
    def test_existing_managed_vpy_gets_new_defaults_and_optional_post_filters(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            vpy_path = Path(temp_dir) / "vpy.vpy"
            vpy_path.write_text(
                'native_kernel = ""  # optional, auto-generated by app\n\n'
                + vpy_edit_and_preview._PREVIOUS_VPY_PROCESSING_SETTINGS
                + vpy_edit_and_preview._GETNATIVE_KERNEL_HELPER
                + vpy_edit_and_preview._VPY_PROCESSING_HELPERS
                + vpy_edit_and_preview._GETNATIVE_DESCALE_BLOCK
                + vpy_edit_and_preview._VPY_FILTER_BLOCK
                + vpy_edit_and_preview._PREVIOUS_VPY_POST_FILTER_BLOCK,
                encoding="utf-8",
            )
            owner = SimpleNamespace(get_default_vpy_path=lambda: str(vpy_path))
            ...
            content = vpy_path.read_text(encoding="utf-8")
            self.assertIn(vpy_edit_and_preview._VPY_PROCESSING_SETTINGS, content)
            self.assertIn(vpy_edit_and_preview._VPY_POST_FILTER_BLOCK, content)
            self.assertNotIn(
                vpy_edit_and_preview._PREVIOUS_VPY_PROCESSING_SETTINGS,
                content,
            )
    ...
    def test_legacy_windows_default_vpy_gets_optional_post_filters(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            vpy_path = Path(temp_dir) / "vpy.vpy"
            vpy_path.write_text(
                'native_kernel = ""  # optional, auto-generated by app\n\n'
                ...
                + vpy_edit_and_preview._PREVIOUS_GETNATIVE_DESCALE_BLOCK
                + vpy_edit_and_preview._LEGACY_VPY_FILTER_BLOCK
                + vpy_edit_and_preview._LEGACY_WINDOWS_VPY_POST_FILTER_BLOCK,
                encoding="utf-8",
            )
            owner = SimpleNamespace(get_default_vpy_path=lambda: str(vpy_path))
    ...
    def test_previous_five_control_vpy_gets_moderate_adaptive_defaults(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            vpy_path = Path(temp_dir) / "vpy.vpy"
            vpy_path.write_text(
                'native_kernel = ""  # optional, auto-generated by app\n\n'
                + vpy_edit_and_preview._PREVIOUS_VPY_PROCESSING_SETTINGS_V2
                + vpy_edit_and_preview._GETNATIVE_KERNEL_HELPER
                + vpy_edit_and_preview._VPY_PROCESSING_HELPERS
                + vpy_edit_and_preview._GETNATIVE_DESCALE_BLOCK
                + vpy_edit_and_preview._VPY_FILTER_BLOCK
                + vpy_edit_and_preview._PREVIOUS_VPY_POST_FILTER_BLOCK_V2,
                encoding="utf-8",
            )
            owner = SimpleNamespace(get_default_vpy_path=lambda: str(vpy_path))
    ...
    def test_customized_previous_vpy_is_not_partially_upgraded(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            content = vpy_path.read_text(encoding="utf-8")
            self.assertIn(customized_helper, content)
            self.assertIn(vpy_edit_and_preview._PREVIOUS_GETNATIVE_DESCALE_BLOCK, content)
            self.assertIn(vpy_edit_and_preview._LEGACY_VPY_FILTER_BLOCK, content)
            self.assertNotIn(vpy_edit_and_preview._VPY_FILTER_BLOCK, content)

    def test_custom_vpy_preview_applies_visible_processing_values(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            vpy_path = Path(temp_dir) / "custom.vpy"
            vpy_path.write_text(
                'a = r"old.mkv"\n'
            ...
            owner = SimpleNamespace(
                _selected_output_bits_for_vpy=lambda: 10,
                _current_vpy_processing_values=lambda: {
                ...
                _patch_fmtc_output_bits_in_text=(
                    vpy_edit_and_preview.VpyEditPreviewMixin._patch_fmtc_output_bits_in_text
                ),
                _patch_vpy_processing_value_in_text=(
                    vpy_edit_and_preview.VpyEditPreviewMixin._patch_vpy_processing_value_in_text
                ),
            )
            owner._vpy_raw_string = lambda path: (
                vpy_edit_and_preview.VpyEditPreviewMixin._vpy_raw_string(owner, path)
            )

            changed = vpy_edit_and_preview.VpyEditPreviewMixin._update_vpy_paths_in_file(
                owner,
                str(vpy_path),
                r"E:\video.mkv",
                "",
    ...
    def test_vpy_loader_converts_rgb_to_luma_without_ffms2(self) -> None:
        ...
        self.assertLess(
            source.index('vs.core.std.LoadAllPlugins(_bluraysub_plugin_path)'),
            source.index('_muvsfunc = importlib.import_module("muvsfunc")'),
        )
        self.assertNotIn('ffms2', source)
        self.assertNotIn('except BaseException', source)
        gui_source = Path(vpy_edit_and_preview.__file__).read_text(encoding="utf-8")
        self.assertIn("environment.insert('BLURAYSUB_PLUGIN_PATH'", gui_source)
