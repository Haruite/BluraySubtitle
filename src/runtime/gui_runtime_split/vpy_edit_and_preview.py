...
from src.runtime.video_timeline import VPY_HARDSUB_CALL, VPY_LEGACY_HARDSUB_CALL
...
class VpyEditPreviewMixin(BluraySubtitleGuiBase):
    ...
    def ensure_default_vpy_file(self):
        ...
        if os.path.exists(path) and os.path.isfile(path):
            try:
                ...
                upgraded = existing.replace(VPY_LEGACY_HARDSUB_CALL, VPY_HARDSUB_CALL)
                if (
    ...
    def set_vpy_hardsub_enabled(self, enabled: bool):
        ...
        for line in lines:
            ...
            if uncommented in (target_1, target_2, VPY_LEGACY_HARDSUB_CALL):
                updated = True
                if uncommented == VPY_LEGACY_HARDSUB_CALL:
